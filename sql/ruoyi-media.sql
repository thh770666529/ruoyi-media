...
CREATE TABLE `blog_article`  (
  ...
  `sort_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '博客分类id',
...
) ENGINE = InnoDB AUTO_INCREMENT = 7 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '博客文章表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of blog_article
-- ----------------------------
INSERT INTO `blog_article` VALUES (4, '121212', '1212', '<p>ShardingSphere分库分表</p><p>==================</p><p><br></p><p>1.什么是垂直/水平拆分模式 2.基于客户端与服务器端实现分表分库区别 3.单表达到多大量开始分表分库 4.数据库分表分|库策略有那些 5.sharding-jdbc实战分库分表 6.MYSQL主从复制 7.Shardingjdbc读写分离 8.Sharding-Proxy 9.Sharding-Sidecar 10.为什么选ShardingSphere作为分库分表中间件而不选Mycat 11.分表分库后如何实现排序查询shardingshere如何实现迁移数据以及扩容的。</p><p><br></p><p>1.数据库分表分库规则:</p><p>------------</p><p><br></p><p>**分库分表**有哪一些**规则**</p><p><br></p><p>**垂直/水平拆分模式**</p><p><br></p><p>**1.垂直模式**指的是从数据结构方面的拆分。一般来说。在系统设计阶段就应该从业务耦合性松紧来确定垂直分库。垂直分表方案。在数据量及访问量不是特别大的情况下。首先考虑缓存。读写分离。索引技术等方案。</p><p><br></p><p>**垂直分库**实际上就是根据公司不同的业务来拆分成不同的数据库 微服务项目架构:分成团队开发模式 也就是我们熟知的分库 会员团队、支付团队、订单团队每个团队中都有自己独立的数据库。会员数据库 订单数据库支付数据库</p><p><br></p><p>**缺点**：1.分布式事务问题。 2.维护成本 3.增大跨库join 4.分布式主键id</p><p><br></p><p>**垂直分表** 可以把一个宽表的字段按照访问频次、是否是大字段的原则拆分为多个表。这样技能业务清晰。还能提升性能。拆分后。尽量从业务的角度避免联查，否则性能方面得不偿失。</p><p><br></p><p>**2.水平拆分:**分为**水平分库**和**水平分表**。就是**同一个库**或者**同一个表**的数据拆分到多个库或者多个表中存放。 单张表数据量超过**500万行**的时候，查询也有可能会遇到瓶颈，分页/排序应该减轻单张表数据量 将一张表数据放入到多个不同的数据库中或者是将一张表数据拆分**n多个子表**存放。</p><p><br></p><p>db\\_01 db\\_02</p><p><br></p><p>**缺点**：查询问题。 分页/排序 内存溢出问题。（shardingjdbc归并数据）</p><p><br></p><p>**3.水平拆分和垂直拆分共同缺点** \\*分布式事务处理困难 \\*夸节点join困难 \\*扩数据源管理复杂</p><p><br></p><p>**4.切分总则** \\*能不切分的尽量不切分 \\*如果要切分，选择合适的切分规则，提前规划好 \\*数据库切分尽量通过数据冗余或表分组来降低跨库join \\*业务尽量使用少的多表join</p><p><br></p><p>2.基于客户端与服务器端实现分表分库区别</p><p>--------------------</p><p><br></p><p>服务器中间件： **mycat**和**sharding-jdbc**和**sharding-proxy**</p><p><br></p><p>**mycat**基于**服务器**端实现数据库代理：</p><p><br></p><p>**官方网站**：[http://www.mycat.org.cn/donate.html](http://www.mycat.org.cn/donate.html)</p><p><br></p><p>![1612094238928](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038785.png)</p><p><br></p><p>底层实际基于服务器实现数据库中间件(mycat)**Mycat**类似于**nginx** **优点**:保证数据库的安全性归并数据结果完全**解耦**</p><p><br></p><p>**缺点：**相对**shardingsphere**来说,软件**更新**速度**慢**。耗损略高</p><p><br></p><p>**shardingjdbc**基于**客户端**实现数据库代理</p><p><br></p><p>![1612257327278](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039034.png)</p><p><br></p><p>基于**客户端**方式实现数据库中间件\\*\\*(ShardingJDBC)\\*\\* **优点**:效率比较高 **缺点**:归并数据结果是没有实现**解耦**，有可能会影响到我们业务逻辑代码。（当归并结果过大时会出现**内存溢出**问题)</p><p><br></p><p>原理：</p><p><br></p><p>Select \\* from user where id=1; 原理:基于aop 代理的方式拦截改写sql语句Select \\* from user\\_1 where id=1 ;</p><p><br></p><p>**sharding-proxy基于服务器端**实现数据库代理(相当于**mycat**)</p><p><br></p><p>![1612256468289](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038964.png)</p><p><br></p><p>**sharding分库分表原理:** SQL解析 -&gt; 查询优化-&gt; SQL路由-&gt;SQL改写-&gt;SQL执行-&gt;结果合并</p><p><br></p><p>3.单表达到多大量开始分表分库</p><p>---------------</p><p><br></p><p>单表行数超过 **500 万**行或者单表容量超过 **2GB**，才推荐进行分库分表。</p><p><br></p><p>说明：如果预计三年后的数据量根本达不到这个级别，请不要在创建表时就分库分表。</p><p><br></p><p>为啥单表行数超过 **500 万**行需要分库分表？</p><p><br></p><p>目前找到了300万数据的表 system\\_user加了主键**b+tree**索引</p><p><br></p><p>    select count(0) from system_user  0.507s     --数据量3323001</p><p>    </p><p>    SELECT  *  FROM system_user LIMIT 3000000,10   --7.497s   </p><p>    </p><p>    SELECT id FROM system_user LIMIT 3000000,10   --0.412s</p><p>    </p><p>    select s.* from system_user s</p><p>    join  (select id from system_user limit 3200000,10) t on t.id = s.id --0.452s</p><p>    </p><p>    --当数据量达到500万的时候 就算是加了索引 也需要将近1s的时间</p><p>    </p><p>    </p><p>    </p><p><br></p><p>为啥单表容量超过 **2GB**，但行数没达到500万也需要分库分表？</p><p><br></p><p>**B+tree**按照**页单位**来读取的，每一页读16kb 如果一行的数据很大的情况下。扫描范围是比较小的， 需要多次读取。所以就算数据库没达到500w行。但是单表数据容量达到2G的时候也需要分表分库。</p><p><br></p><p>4.数据库分表分|库策略有那些</p><p>---------------</p><p><br></p><p>**取余/取模** 优点：均匀存放数据 缺点：不能扩容</p><p><br></p><p>**按照范围分片** 1-500万 501-1000万 优点：容易扩容，实现简单。</p><p><br></p><p>**按照日期进行分片** 日志 订单信息。</p><p><br></p><p>**按照月份进行分片** 保险业务。</p><p><br></p><p>**按照枚举值分片** 常量分片 比如。省份分表</p><p><br></p><p>**二进制取模范围分片**</p><p><br></p><p>**一致性hash分片** 类似hashmap 缺点： 数据存放不均匀。</p><p><br></p><p>**按照目标字段前缀指定的进行分区**</p><p><br></p><p>**按照前缀ASCII码和值进行取模范围分片**</p><p><br></p><p>预定义：适合小型 反推数据 几年下不会太多数据。--&gt;一般是创业型公司。</p><p><br></p><p>**主流分片算法**： 取余/取模 日期 一致性hash分片</p><p><br></p><p>根据user id</p><p><br></p><p>user id=1%2=1</p><p><br></p><p>user id=0%2=0</p><p><br></p><p>user id=3%2=3</p><p><br></p><p>user id=4%2=4</p><p><br></p><p>不能实现扩容。</p><p><br></p><p>5.Sharding-jdbc实战分库分表</p><p>---------------------</p><p><br></p><p>**官方网站**：[http://shardingsphere.apache.org/index\\_zh.html](http://shardingsphere.apache.org/index_zh.html)</p><p><br></p><p>**文档**：[https://shardingsphere.apache.org/document/current/cn/overview/](https://shardingsphere.apache.org/document/current/cn/overview/)</p><p><br></p><p>概述：定位为轻量级Java框架，在Java的JDBC层提供的额外服务。 它使用客户端直连数据库，以jar包形式提供服务，无需额外部署和依赖，可理解为增强版的JDBC驱动，完全兼容JDBC和各种ORM框架。</p><p><br></p><p>*   适用于任何基于JDBC的ORM框架，如：JPA, Hibernate, Mybatis, Spring JDBC Template或直接使用JDBC。</p><p>*   支持任何第三方的数据库连接池，如：DBCP, C3P0, BoneCP, Druid, HikariCP等。</p><p>*   支持任意实现JDBC规范的数据库。目前支持MySQL，Oracle，SQLServer，PostgreSQL以及任何遵循SQL92标准的数据库。</p><p><br></p><p>![1612257304024](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039027.png)</p><p><br></p><p>![1612101183082](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038817.png)</p><p><br></p><p>* * *</p><p><br></p><p>目前我实战采用的是单实例 多表的情况 也就是水平分表的情况。</p><p><br></p><p>**0.创建数据库**</p><p><br></p><p>    SET NAMES utf8mb4;</p><p>    SET FOREIGN_KEY_CHECKS = 0;</p><p>    </p><p>    DROP database IF EXISTS `shardingsphere_demo`;</p><p>    </p><p>    CREATE DATABASE `shardingsphere_demo` CHARACTER SET = utf8 ;</p><p>    </p><p>    use `shardingsphere_demo`;</p><p>    -- ----------------------------</p><p>    -- Table structure for t_user_0</p><p>    -- ----------------------------</p><p>    DROP TABLE IF EXISTS `t_user_0`;</p><p>    CREATE TABLE `t_user_0`  (</p><p>      `id` bigint(0) NOT NULL COMMENT \'主键\',</p><p>      `name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT \'姓名\',</p><p>      `age` int(0) NULL DEFAULT NULL COMMENT \'年龄\',</p><p>      `sex` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT \'性别\',</p><p>      PRIMARY KEY (`id`) USING BTREE</p><p>    ) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;</p><p>    </p><p>    -- ----------------------------</p><p>    -- Records of t_user_0</p><p>    -- ----------------------------</p><p>    INSERT INTO `t_user_0` VALUES (1, \'张三\', 18, \'1\');</p><p>    INSERT INTO `t_user_0` VALUES (3, \'王五\', 21, \'0\');</p><p>    </p><p>    -- ----------------------------</p><p>    -- Table structure for t_user_1</p><p>    -- ----------------------------</p><p>    DROP TABLE IF EXISTS `t_user_1`;</p><p>    CREATE TABLE `t_user_1`  (</p><p>      `id` bigint(0) NOT NULL COMMENT \'主键\',</p><p>      `name` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT \'姓名\',</p><p>      `age` int(0) NULL DEFAULT NULL COMMENT \'年龄\',</p><p>      `sex` char(1) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT \'性别\',</p><p>      PRIMARY KEY (`id`) USING BTREE</p><p>    ) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;</p><p>    </p><p>    -- ----------------------------</p><p>    -- Records of t_user_1</p><p>    -- ----------------------------</p><p>    INSERT INTO `t_user_1` VALUES (2, \'李四\', 20, \'0\');</p><p>    INSERT INTO `t_user_1` VALUES (4, \'赵六\', 17, \'1\');</p><p>    </p><p>    SET FOREIGN_KEY_CHECKS = 1;</p><p>    </p><p>    </p><p><br></p><p>**1.创建项目**</p><p><br></p><p>![1612147645376](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038838.png)</p><p><br></p><p>**2.pom.xml**依赖</p><p><br></p><p>    &lt;?xml version=\"1.0\" encoding=\"UTF-8\"?&gt;</p><p>    &lt;project xmlns=\"http://maven.apache.org/POM/4.0.0\"</p><p>             xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"</p><p>             xsi:schemaLocation=\"http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd\"&gt;</p><p>        &lt;modelVersion&gt;4.0.0&lt;/modelVersion&gt;</p><p>    </p><p>        &lt;parent&gt;</p><p>            &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>            &lt;artifactId&gt;spring-boot-starter-parent&lt;/artifactId&gt;</p><p>            &lt;version&gt;2.3.1.RELEASE&lt;/version&gt;</p><p>            &lt;relativePath/&gt;</p><p>        &lt;/parent&gt;</p><p>        &lt;description&gt; 学习shardingjdbc分库分表&lt;/description&gt;</p><p>    </p><p>        &lt;groupId&gt;com.thh&lt;/groupId&gt;</p><p>        &lt;artifactId&gt;shardingsphere-demo&lt;/artifactId&gt;</p><p>        &lt;version&gt;1.0.0-SNAPSHOT&lt;/version&gt;</p><p>    </p><p>        &lt;properties&gt;</p><p>            &lt;java.version&gt;1.8&lt;/java.version&gt;</p><p>            &lt;shardingsphere.version&gt;4.0.0-RC1&lt;/shardingsphere.version&gt;</p><p>        &lt;/properties&gt;</p><p>    </p><p>        &lt;dependencies&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;spring-boot-starter-test&lt;/artifactId&gt;</p><p>                &lt;scope&gt;test&lt;/scope&gt;</p><p>                &lt;exclusions&gt;</p><p>                    &lt;exclusion&gt;</p><p>                        &lt;groupId&gt;org.junit.vintage&lt;/groupId&gt;</p><p>                        &lt;artifactId&gt;junit-vintage-engine&lt;/artifactId&gt;</p><p>                    &lt;/exclusion&gt;</p><p>                &lt;/exclusions&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;!-- mysql5.7驱动 --&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;mysql&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;mysql-connector-java&lt;/artifactId&gt;</p><p>                &lt;version&gt;5.1.4&lt;/version&gt;</p><p>                &lt;scope&gt;runtime&lt;/scope&gt;</p><p>            &lt;/dependency&gt;</p><p>            &lt;!-- jsonson处理，服务于jsonutil工具和springmvc的json返回 --&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;com.fasterxml.jackson.dataformat&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;jackson-dataformat-avro&lt;/artifactId&gt;</p><p>                &lt;version&gt;2.10.0&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;!-- md5密码加密的时依赖的包 --&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;commons-codec&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;commons-codec&lt;/artifactId&gt;</p><p>                &lt;version&gt;1.11&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;com.alibaba&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;druid-spring-boot-starter&lt;/artifactId&gt;</p><p>                &lt;version&gt;1.1.10&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;spring-boot-starter-aop&lt;/artifactId&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;spring-boot-starter-web&lt;/artifactId&gt;</p><p>            &lt;/dependency&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;spring-boot-devtools&lt;/artifactId&gt;</p><p>                &lt;scope&gt;runtime&lt;/scope&gt;</p><p>                &lt;optional&gt;true&lt;/optional&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.apache.shardingsphere&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;sharding-jdbc-spring-boot-starter&lt;/artifactId&gt;</p><p>                &lt;version&gt;${shardingsphere.version}&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>            &lt;!--mybatis-plus整合mybatis--&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;com.baomidou&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;mybatis-plus-boot-starter&lt;/artifactId&gt;</p><p>                &lt;version&gt;3.4.2&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;com.alibaba&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;fastjson&lt;/artifactId&gt;</p><p>                &lt;version&gt;1.2.62&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>    </p><p>    </p><p>    </p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.projectlombok&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;lombok&lt;/artifactId&gt;</p><p>            &lt;/dependency&gt;</p><p>            &lt;dependency&gt;</p><p>                &lt;groupId&gt;org.apache.commons&lt;/groupId&gt;</p><p>                &lt;artifactId&gt;commons-lang3&lt;/artifactId&gt;</p><p>                &lt;version&gt;3.6&lt;/version&gt;</p><p>            &lt;/dependency&gt;</p><p>        &lt;/dependencies&gt;</p><p>        &lt;build&gt;</p><p>            &lt;finalName&gt;${project.artifactId}&lt;/finalName&gt;</p><p>            &lt;plugins&gt;</p><p>                &lt;plugin&gt;</p><p>                    &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;</p><p>                    &lt;artifactId&gt;spring-boot-maven-plugin&lt;/artifactId&gt;</p><p>                &lt;/plugin&gt;</p><p>            &lt;/plugins&gt;</p><p>        &lt;/build&gt;</p><p>    &lt;/project&gt;</p><p>    </p><p>    </p><p><br></p><p>**3.aplication.yml**文件</p><p><br></p><p>    server:</p><p>      port: 8999</p><p>    </p><p>    </p><p>    logging:</p><p>      level:</p><p>        root: info</p><p>    </p><p>    </p><p>    </p><p>    spring:</p><p>      profiles:</p><p>        active: dev</p><p>      main:   #允许相同bean名称覆盖</p><p>        allow-bean-definition-overriding: true</p><p>      application:</p><p>        name: shardingsphere-demo</p><p>      jackson:</p><p>        # 日期进行格式化处理</p><p>        date-format: yyyy/MM/dd HH:mm:ss</p><p>        # 解决json转换少8个小时的问题</p><p>        time-zone: GMT+8</p><p>        # 解决json返回过程中long的精度丢失问题</p><p>        generator:</p><p>          write-numbers-as-strings: true</p><p>          write-bigdecimal-as-plain: true</p><p>    </p><p>    </p><p>    # mybatis-plus配置</p><p>    mybatis-plus:</p><p>      mapper-locations: classpath*:/mappers/*.xml</p><p>      configuration:</p><p>        call-setters-on-nulls: true</p><p>      type-aliases-package: com.thh.entity</p><p>    </p><p>    </p><p>    </p><p>    </p><p>    </p><p><br></p><p>**application-dev.yml**</p><p><br></p><p>    spring:</p><p>      shardingsphere:</p><p>        datasource:</p><p>          names: ds0</p><p>          ds0:</p><p>            driver-class-name: com.mysql.jdbc.Driver</p><p>            url: jdbc:mysql://localhost:3306/shardingsphere_demo?useUnicode=true&amp;characterEncoding=UTF-8&amp;serverTimezone=GMT&amp;useSSL=false</p><p>            password: 123456</p><p>            type: com.alibaba.druid.pool.DruidDataSource</p><p>            username: root</p><p>    </p><p>    </p><p>        sharding:</p><p>          binding-tables: t_user</p><p>          default-database-strategy:</p><p>            inline:</p><p>              algorithm-expression: ds0</p><p>              sharding-column: user_id</p><p>          tables:</p><p>            t_user:</p><p>              actual-data-nodes: ds0.t_user_$-&gt;{0..1}</p><p>              key-generator:</p><p>                column: id</p><p>                type: SNOWFLAKE</p><p>              table-strategy:</p><p>                inline:</p><p>                  algorithm-expression: t_user_$-&gt;{id % 2 +1}</p><p>                  sharding-column: id</p><p>        props:</p><p>          sql:</p><p>            show:  true</p><p>    </p><p>    </p><p>    </p><p>    </p><p><br></p><p>**4**.创建**实体类** **controller** 以及**mapper**</p><p><br></p><p>**启动类**</p><p><br></p><p>    /**</p><p>     * @ClassName ShardingsphereDemoApplication</p><p>     * @Description 启动类</p><p>     * @Author thh</p><p>     * @Date 0:03 2021/02/01</p><p>     **/</p><p>    @SpringBootApplication</p><p>    @MapperScan(\"com.thh.mapper\")</p><p>    public class ShardingsphereDemoApplication {</p><p>        public static void main(String[] args) {</p><p>            SpringApplication.run(ShardingsphereDemoApplication.class,args);</p><p>        }</p><p>    }</p><p>    </p><p><br></p><p>**entity实体类**</p><p><br></p><p>    @Data</p><p>    @ToString</p><p>    @NoArgsConstructor</p><p>    @AllArgsConstructor</p><p>    @TableName(\"t_user\")</p><p>    public class User {</p><p>        // 主键</p><p>        @TableId</p><p>        private Long id;</p><p>        // 用户名</p><p>        private String name;</p><p>    </p><p>        // 年龄</p><p>        private Integer age;</p><p>    </p><p>        /**</p><p>         * 性别 0代表男 1代表女</p><p>         */</p><p>        private String sex;</p><p>    </p><p>    </p><p>    }</p><p>    </p><p><br></p><p>**mapper**</p><p><br></p><p>    @Repository</p><p>    public interface UserMapper extends BaseMapper&lt;User&gt; {</p><p>        /**</p><p>         * 查询所有用户</p><p>         * @return</p><p>         */</p><p>        @Select(\"select * from t_user\")</p><p>       List&lt;User&gt; selectUserList();</p><p>    }</p><p>    </p><p><br></p><p>**controller**</p><p><br></p><p>    </p><p>    /**</p><p>     * @ClassName UserController</p><p>     * @Description web</p><p>     * @Author thh</p><p>     * @Date 0:09 2021/02/01</p><p>     **/</p><p>    @RestController</p><p>    @RequestMapping(\"/user\")</p><p>    public class UserController {</p><p>        @Autowired</p><p>        private UserMapper userMapper;</p><p>    </p><p>        /**</p><p>         * 查询所有用户</p><p>         * @return</p><p>         */</p><p>        @RequestMapping(\"/list\")</p><p>        public List&lt;User&gt; getUserList(){</p><p>            return userMapper.selectUserList();</p><p>        }</p><p>    }</p><p>    </p><p><br></p><p>实际上我们的表 是 t\\_user\\_0 t\\_user\\_1 shardingshere帮我们通过查询 t\\_user虚拟表 然后帮我们映射成实际的sql，把查出来的数据进行了归并。</p><p><br></p><p>![1612147884181](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038845.png)</p><p><br></p><p>**公共表**的配置</p><p><br></p><p>**公共表**属于系统中数据量较小,变动少,而且属于**高频联合查询**的**依赖表**。**参数表**、**数据字典表**等属于此类型。可以将这类表在每个数据库都保存一份,所有更新操作都同时发送到所有分库执行。接下来看一下如何使用Sharding-IDBC实现公共表。|</p><p><br></p><p>如果是实现的是多实例。**公共表**的情况的话 比如**字典表**。每个实例都需要用的情况，我们就会配置公共表.</p><p><br></p><p>sharding-jdbc帮我们做的就是路由到各个数据源。当我们进行插入操作的时候。把数据插入到不同数据源的公共表当中。</p><p><br></p><p>    spring:</p><p>      shardingsphere:</p><p>        datasource:</p><p>          names: ds0</p><p>          ds0:</p><p>            driver-class-name: com.mysql.jdbc.Driver</p><p>            url: jdbc:mysql://47.115.49.189:3306/shardingsphere_demo?useUnicode=true&amp;characterEncoding=UTF-8&amp;serverTimezone=GMT&amp;useSSL=false</p><p>            password: mogu2018</p><p>            type: com.alibaba.druid.pool.DruidDataSource</p><p>            username: root</p><p>    </p><p>    </p><p>        sharding:</p><p>          binding-tables: t_user  #绑定的表</p><p>          default-database-strategy:</p><p>            inline:</p><p>              algorithm-expression: ds0</p><p>              sharding-column: user_id</p><p>          tables:</p><p>            t_user:</p><p>              actual-data-nodes: ds0.t_user_$-&gt;{0..1}</p><p>              key-generator:</p><p>                column: id</p><p>                type: SNOWFLAKE</p><p>              table-strategy:</p><p>                inline:</p><p>                  algorithm-expression: t_user_$-&gt;{id % 2 +1}</p><p>                  sharding-column: id</p><p>          broadcast-tables: t_dist   #公共表配置 </p><p>        props: #开启sql输出日志</p><p>          sql:</p><p>            show:  true</p><p>    </p><p>    </p><p>    </p><p>    </p><p><br></p><p>6.MYSQL主从复制(读写分离前提)</p><p>-------------------</p><p><br></p><p>**理解读写分离**</p><p><br></p><p>面对日益增加的系统访问量,数据库的吞吐量面临着巨大瓶颈。对于同一 时刻有大量并发读操作和较少写操作类型的应用系统来说,将数据库拆分为主库和从库,主库负责处理事务性的增删改操作,从库负责处理查询操作,能够有效的避免由数据更新导致的行锁,使得整个系统的查询性能得到极大的改善。</p><p><br></p><p>通过一主多从的配置方式,可以将查询请求均匀的分散到多个数据副本,能够进一步的提升 系统的处理能力。使用多主多从的方式 ,不但能够提升系统的吞吐量,还能够提升系统的可用性,可以达到在任何-一个数据库宕机,甚至磁盘物理损坏的情况下仍然不影响系统的正常运行。</p><p><br></p><p>主从同步的N种实现方式?**主从同步**其实与**读写分离**是两码事 **Sharding-JDBC**读写分离则是根据SQL语义的分析,将读操作和写操作分别路由至主库与从库。 它提供透明化读写分离,让使用方尽量像使用一个数据库-样使用主从数据库集群。</p><p><br></p><p>![1612181228861](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038852.png)</p><p><br></p><p>**Sharding-IDBC**提供- -主多从的读写分离配置,可独立使用,也可配合分库分表使用,同一线程且同- -数据库连接内,如有写入操作,以后的读操作均从主库读取,用于保证数据一致性。 **Sharding-JDBC**不提供主从数据库的数据同步功能 ,需要采用其他机制支持。</p><p><br></p><p>**MYSQL**自带**主从同步**机制。</p><p><br></p><p>**主从复制工作原理剖析**</p><p><br></p><p>&gt; 1.Master 数据库只要发生变化，立马记录到Binary log 日志文件中(binlog 手动开启)</p><p>&gt; </p><p>&gt; 2.Slave数据库启动一个I/O thread连接Master数据库，请求Master变化的二进制日志 3.Slave I/O获取到的二进制日志，保存到自己的Relay log 日志文件中。(Relay log 中继日志 持久化存储) 4.Slave 有一个 SQL thread定时检查Realy log是否变化，变化那么就更新数据</p><p><br></p><p>**存在的问题：1.2.3部**操作都是顺序操作。执行很快。**第4部重放**会特别慢。重放的时候 解析出来的DML语句。mysql执行DML语句的时候。比如 update table set name =‘111’ where id =‘1’ update table set name =‘123’ where id =‘200’.</p><p><br></p><p>实际上更新的块是不同的。执行速度慢，这会造成relay log 堆积。出现**主从复制的延时问题**。</p><p><br></p><p>\\*\\*理解：\\*\\*说白了就是日志监测。只要有日志。说明数据有变化 主就往从更新数据。</p><p><br></p><p>![1612182222659](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038858.png)</p><p><br></p><p>主从配置需要**注意**的点</p><p><br></p><p>(1)主从服务器操作系统版本和位数一致；</p><p><br></p><p>(2) Master和Slave数据库的版本要一致；</p><p><br></p><p>(3) Master和Slave数据库中的数据要一致；</p><p><br></p><p>(4) Master开启二进制日志，Master和Slave的server\\_id在局域网内必须唯一；</p><p><br></p><p>**01.环境准备**：首先搭建**两个mysql服务**。可以是linux方式 docker方式或者是windows方式。两个服务器可以ping通。</p><p><br></p><p>docker方式创建</p><p><br></p><p>    #主mysql</p><p>    docker run  -di --name=mysql_master --restart always -v /docker/mysql_master/data:/var/lib/mysql -v /docker/mysql_master/config:/etc/mysql   -p  3307:3306 -e MYSQL_ROOT_PASSWORD=sharding123 mysql:5.7</p><p>    </p><p>    #需要在config下复制mysql.cnf 为my.cnf 不然配置不会生效</p><p>    cd /docker/mysql_master/config</p><p>    cp mysql.cnf my.cnf</p><p>    </p><p>    </p><p>    #从mysql</p><p>    docker run  -di --name=mysql_slave --restart always -v /docker/mysql_slave/data:/var/lib/mysql -v /docker/mysql_slave/config:/etc/mysql   -p  3308:3306 -e MYSQL_ROOT_PASSWORD=sharding123 mysql:5.7</p><p>    </p><p>    cd /docker/mysql_slave/config</p><p>    cp mysql.cnf my.cnf</p><p>    </p><p><br></p><p>![1612189573177](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038865.png)</p><p><br></p><p>![1612191701217](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038888.png)</p><p><br></p><p>**02.开启mysql的binlog日志功能**</p><p><br></p><p>**1.**修改**主和从**的**my.cnf**文件</p><p><br></p><p>**主配置**</p><p><br></p><p>    #mysql master1 config </p><p>    [mysqld]</p><p>    server-id = 1        # 节点ID，确保唯一 一般设置为IP</p><p>    binlog-do-db=shardingsphere_demo  # 复制过滤：需要备份的数据库，输出binlog</p><p>    </p><p>    </p><p>    # log config</p><p>    log-bin = mysql-bin        #开启mysql的binlog日志功能 可以随便取，最好有含义</p><p>    sync_binlog = 1            #控制数据库的binlog刷到磁盘上去 , 0 不控制，性能最好，1每次事物提交都会刷到日志文件中，性能最差，最安全</p><p>    binlog_format = mixed      #binlog日志格式，mysql默认采用statement，建议使用mixed</p><p>    expire_logs_days = 7       #binlog过期清理时间 二进制日志自动删除/过期的天数。默认值为0，表示不自动删除</p><p>    max_binlog_size = 100m     #binlog每个日志文件大小</p><p>    binlog_cache_size = 4m     #binlog缓存大小  为每个session 分配的内存，在事务过程中用来存储二进制日志的缓存</p><p>    max_binlog_cache_size= 512m   #最大binlog缓存大</p><p>    binlog-ignore-db=mysql     #不需要备份的数据库不生成日志文件的数据库，多个忽略数据库可以用逗号拼接，或者 复制这句话，写多行</p><p>    </p><p>    ## 如：1062错误是指一些主键重复，1032错误是因为主从数据库数据不一致</p><p>    ## 跳过主从复制中遇到的所有错误或指定类型的错误，避免slave端复制中断。</p><p>    #slave-skip-errors = all #跳过从库错误</p><p>    slave-skip-errors = all </p><p>    </p><p>    </p><p>    </p><p>    auto-increment-offset = 1     # 自增值的偏移量</p><p>    auto-increment-increment = 1  # 自增值的自增量</p><p>    </p><p>    </p><p><br></p><p>**从配置**</p><p><br></p><p>    #mysql slave config </p><p>    [mysqld]</p><p>    server-id = 2</p><p>    log-bin=mysql-bin</p><p>    relay-log = mysql-relay-bin</p><p>    replicate-wild-ignore-table=mysql.%</p><p>    replicate-wild-ignore-table=test.%</p><p>    replicate-wild-ignore-table=information_schema.%</p><p>    </p><p><br></p><p>**MySQL**对于二进制日志 (**binlog**)的复制类型</p><p><br></p><p>(1) 基于语句的复制：在Master上执行的SQL语句，在Slave上执行同样的语句。MySQL默认采用基于语句的复制，效率比较高。一旦发现没法精确复制时，会自动选着基于行的复制。</p><p><br></p><p>(2) 基于行的复制：把改变的内容复制到Slave，而不是把命令在Slave上执行一遍。从MySQL5.0开始支持。</p><p><br></p><p>(3) 混合类型的复制：默认采用基于语句的复制，一旦发现基于语句的无法精确的复制时，就会采用基于行的复制。</p><p><br></p><p>修改完了 别忘记重启</p><p><br></p><p>docker restart mysql\\_master</p><p><br></p><p>docker restart mysql\\_slave</p><p><br></p><p>**2.**可以创建**另外的用户**进行测试。或者直接拿root用户进行测试。</p><p><br></p><p>    -- 创建用户</p><p>    CREATE USER sharding IDENTIFIED BY \'sharding123\';</p><p>    </p><p>    -- 赋予权限</p><p>    grant replication slave on *.* to \'sharding\'@\'%\'  identified by \'sharding123\';</p><p>    </p><p>    -- 生效</p><p>    FLUSH PRIVILEGES;</p><p>    </p><p><br></p><p>**3**.查看**master**的状态</p><p><br></p><p>    --先进入容器</p><p>     docker exec -it mysql_master /bin/bash</p><p>    --连接mysql</p><p>    mysql -uroot -psharding123;</p><p>    mysql&gt; show master status;</p><p>    +------------------+----------+--------------+------------------+-------------------+</p><p>    | File             | Position | Binlog_Do_DB | Binlog_Ignore_DB | Executed_Gtid_Set |</p><p>    +------------------+----------+--------------+------------------+-------------------+</p><p>    | mysql-bin.000004      154   |              | mysql            |                   |</p><p>    +------------------+----------+--------------+------------------+-------------------+</p><p>    1 row in set (0.00 sec)</p><p>    </p><p><br></p><p>![1612235805770](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038906.png)</p><p><br></p><p>**4.从节点配置master配置**。（小弟认大哥的过程）</p><p><br></p><p>    -- master的配置 </p><p>    --master_log_pos对应的是 show master status 查出的 position</p><p>    --master_log_file对应的是 show master status 查出的 file    这两个都需要要对上了 才会同步成成功</p><p>    </p><p>    change master to master_host=\'172.18.7.9\',master_port=3307,master_user=\'sharding\',master_password=\'sharding123\',master_log_file=\'mysql-bin.000004\',master_log_pos=154;</p><p>    -- 开启SLAVE线程：</p><p>    start slave;</p><p>    </p><p><br></p><p>![1612236922306](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038913.png)</p><p><br></p><p>![1612237276983](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038919.png)</p><p><br></p><p>**5.效验是否进行了同步**</p><p><br></p><p>![1612237659849](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038926.png)</p><p><br></p><p>测试成功。到master数据库的shadingshere\\_demo库进行增删改操作的时候。从节点的mysql也会对shadingshere\\_demo库进行操作。</p><p><br></p><p>7.Sharding-jdbc读写分离</p><p>-------------------</p><p><br></p><p>**官方文档地址**：[https://shardingsphere.apache.org/document/legacy/4.x/document/cn/manual/sharding-jdbc/configuration/config-spring-boot/](https://shardingsphere.apache.org/document/legacy/4.x/document/cn/manual/sharding-jdbc/configuration/config-spring-boot/)</p><p><br></p><p>![1612254786106](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038940.png)</p><p><br></p><p>    </p><p>    spring:        #实现读写分离 配置主从库</p><p>      shardingsphere:</p><p>        datasource:</p><p>          names: master,slave0</p><p>          master:</p><p>            driver-class-name: com.mysql.jdbc.Driver</p><p>            url: jdbc:mysql://localhost:3307/shardingsphere_demo?useUnicode=true&amp;characterEncoding=UTF-8&amp;serverTimezone=GMT&amp;useSSL=false</p><p>            username: sharding</p><p>            password: sharding123</p><p>            type: com.alibaba.druid.pool.DruidDataSource</p><p>    </p><p>          slave0:</p><p>            driver-class-name: com.mysql.jdbc.Driver</p><p>            url: jdbc:mysql://localhost:3308/shardingsphere_demo?useUnicode=true&amp;characterEncoding=UTF-8&amp;serverTimezone=GMT&amp;useSSL=false</p><p>            username: root</p><p>            password: sharding123</p><p>            type: com.alibaba.druid.pool.DruidDataSource</p><p>    </p><p>    </p><p>    </p><p>        sharding:</p><p>          master-slave-rules:</p><p>            ds0:</p><p>              master-data-source-name:  master</p><p>              slave-data-source-names:  slave0</p><p>          binding-tables: t_user  #绑定的表</p><p>          default-database-strategy:</p><p>            inline:</p><p>              algorithm-expression: ds0</p><p>              sharding-column: user_id</p><p>          tables:</p><p>            t_user:</p><p>              actual-data-nodes: ds0.t_user_$-&gt;{0..1}</p><p>              key-generator:</p><p>                column: id</p><p>                type: SNOWFLAKE</p><p>              table-strategy:</p><p>                inline:</p><p>                  algorithm-expression: t_user_$-&gt;{id % 2}</p><p>                  sharding-column: id</p><p>          #broadcast-tables: t_dist   #公共表配置</p><p>    </p><p>        props: #开启sql输出日志</p><p>          sql:</p><p>            show:  true</p><p>    </p><p>    </p><p>    </p><p>    </p><p><br></p><p>写操作路由的是主**master**数据库</p><p><br></p><p>![1612256011081](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038947.png)</p><p><br></p><p>读操作路由到的是**slave0**数据库</p><p><br></p><p>![1612256169991](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038957.png)</p><p><br></p><p>8.Sharding-Proxy</p><p>----------------</p><p><br></p><p>**1.概述**：定位为透明化的数据库代理端，提供封装了数据库二进制协议的服务端版本，用于完成对异构语言的支持。 目前先提供MySQL/PostgreSQL版本，它可以使用任何兼容MySQL/PostgreSQL协议的访问客户端(如：MySQL Command Client, MySQL Workbench, Navicat等)操作数据，对DBA更加友好。</p><p><br></p><p>基于**服务器端**的**数据库代理**中间件 类似**mycat**</p><p><br></p><p>![1612256570203](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275038970.png)</p><p><br></p><p>**2.下载**</p><p><br></p><p>![1612256844202](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039008.png)</p><p><br></p><p>也可以用官方docker镜像 [https://shardingsphere.apache.org/document/legacy/4.x/document/cn/manual/sharding-proxy/docker/](https://shardingsphere.apache.org/document/legacy/4.x/document/cn/manual/sharding-proxy/docker/)</p><p><br></p><p>![1612271338599](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039204.png)</p><p><br></p><p>**手动下载压缩包。**</p><p><br></p><p>![1612256969373](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039021.png)</p><p><br></p><p>**3.安装。下载完之后进行解压。启动bin目录文件就可以了。**</p><p><br></p><p>**解压**的时候不要有**中文**目录和**空格**。</p><p><br></p><p>![1612258678268](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039042.png)</p><p><br></p><p>解压下来后lib下的**jar包**修改一下后缀名。全部改成**jar** 有**部分名称**缺了。**前缀**为**sharding**的**后缀**统一为\\*\\*-4.0.1.jar\\*\\*</p><p><br></p><p>![1612259051595](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039050.png)</p><p><br></p><p>![1612270846417](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039180.png)</p><p><br></p><p>**4.Sharding-Proxy配置**</p><p><br></p><p>1.  进入conf目录修改server.yaml文件 把**两段注释**去掉</p><p>    </p><p>    ![1612259772988](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039057.png)</p><p>    </p><p>2.  进入conf目录。修改config-sharding.yaml</p><p>    </p><p><br></p><p>还是去掉注释。修改成我们对应的库就可以了。和sharding-jdbc差不多。</p><p><br></p><p>放mysql驱动到lib下</p><p><br></p><p>![1612260548771](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039064.png)</p><p><br></p><p>![1612260675043](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039071.png)</p><p><br></p><p>    schemaName: sharding_db</p><p>    </p><p>    dataSources:</p><p>      ds_0:</p><p>        url: jdbc:mysql://localhost:3306/shardingsphere_demo?serverTimezone=UTC&amp;useSSL=false</p><p>        username: root</p><p>        password: 123456</p><p>        connectionTimeoutMilliseconds: 30000</p><p>        idleTimeoutMilliseconds: 60000</p><p>        maxLifetimeMilliseconds: 1800000</p><p>        maxPoolSize: 50</p><p>    </p><p>    </p><p>    shardingRule:</p><p>      tables:</p><p>        t_user:</p><p>          actualDataNodes: ds_${0}.t_user_${0..1}</p><p>          tableStrategy:</p><p>            inline:</p><p>              shardingColumn: order_id</p><p>              algorithmExpression: t_user_${id % 2}</p><p>          keyGenerator:</p><p>            type: SNOWFLAKE</p><p>            column: id</p><p>      bindingTables:</p><p>        - t_user</p><p>      defaultDatabaseStrategy:</p><p>        inline:</p><p>          shardingColumn: user_id</p><p>          algorithmExpression: ds_${0}</p><p>      defaultTableStrategy:</p><p>        none:</p><p>    </p><p><br></p><p>**5.启动**</p><p><br></p><p>bin目录下执行 默认端口3307 如果想**指定端口** 就直接 cmd打开黑框 start.bat 端口号</p><p><br></p><p>![1612260789305](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039078.png)</p><p><br></p><p>由于路径上有空格 出现了以下报错。找不到文件路径</p><p><br></p><p>![1612260926336](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039094.png)</p><p><br></p><p>换成**没有空格**的目录后执行。虽然执行成功了 还是报了另外一个错误。</p><p><br></p><p>![1612261283068](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039101.png)</p><p><br></p><p>后面看了之后找到原因。是因为mysql驱动过低的问题</p><p><br></p><p>替换**mysql8.0**的驱动后解决问题。</p><p><br></p><p>![1612262116385](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039108.png)</p><p><br></p><p>![](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039116.png)</p><p><br></p><p>果然和**mycat**没啥区别。不过好的就是文档写的还是特别到位。目前捐给apache管理。</p><p><br></p><p>**6.sharding-proxy配置读写分离**</p><p><br></p><p>老规矩。打开**config-master\\_slave.yaml** 去掉注释。</p><p><br></p><p>![1612265681430](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039124.png)</p><p><br></p><p>**读写分离**配置</p><p><br></p><p>    schemaName: master_slave_db</p><p>    </p><p>    dataSources:</p><p>      master_ds:</p><p>        url: jdbc:mysql://localhost:3307/shardingsphere_demo?serverTimezone=UTC&amp;useSSL=false</p><p>        username: sharding</p><p>        password: sharding123</p><p>        connectionTimeoutMilliseconds: 30000</p><p>        idleTimeoutMilliseconds: 60000</p><p>        maxLifetimeMilliseconds: 1800000</p><p>        maxPoolSize: 50</p><p>      slave_ds_0:</p><p>        url: jdbc:mysql://localhost:3308/shardingsphere_demo?serverTimezone=UTC&amp;useSSL=false</p><p>        username: sharding</p><p>        password: sharding123</p><p>        connectionTimeoutMilliseconds: 30000</p><p>        idleTimeoutMilliseconds: 60000</p><p>        maxLifetimeMilliseconds: 1800000</p><p>        maxPoolSize: 50</p><p>    </p><p>    </p><p>    masterSlaveRule:</p><p>      name: ms_ds</p><p>      masterDataSourceName: master_ds</p><p>      slaveDataSourceNames:</p><p>        - slave_ds_0</p><p>    </p><p><br></p><p>![1612266681135](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039140.png)</p><p><br></p><p>配置完之后,用客户端**navicat12**又出现了另外一个报错 ,好家伙。</p><p><br></p><p>![1612267890459](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039159.png)</p><p><br></p><p>然后我通过**黑框**客户端去连接后，我发现是可以进行操作的。所以实际上是没问题的。是我的那**navicat12**不兼容sharding-proxy。</p><p><br></p><p>![1612270254883](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039165.png)</p><p><br></p><p>这样 我做一下**查询更新**的操作 看看打印出的sql 会不会做到**自动路由**达到**读写分离**</p><p><br></p><p>![1612270688134](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039172.png)</p><p><br></p><p>**读写分离+数据分片配置**</p><p><br></p><p>    schemaName: sharding_master_slave_db</p><p>    </p><p>    dataSources:</p><p>      ds0:</p><p>        url: jdbc:mysql://localhost:3307/shardingsphere_demo?serverTimezone=UTC&amp;useSSL=false</p><p>        username: sharding</p><p>        password: sharding123</p><p>        connectionTimeoutMilliseconds: 30000</p><p>        idleTimeoutMilliseconds: 60000</p><p>        maxLifetimeMilliseconds: 1800000</p><p>        maxPoolSize: 50</p><p>      ds0_slave0:</p><p>        url: jdbc:mysql://localhost:3308/shardingsphere_demo?serverTimezone=UTC&amp;useSSL=false</p><p>        username: sharding</p><p>        password: sharding123</p><p>        connectionTimeoutMilliseconds: 30000</p><p>        idleTimeoutMilliseconds: 60000</p><p>        maxLifetimeMilliseconds: 1800000</p><p>        maxPoolSize: 50</p><p>    </p><p>    </p><p>    shardingRule:  </p><p>      tables:</p><p>        t_user: </p><p>          actualDataNodes: ms_ds${0}.t_user_${0..1}</p><p>          databaseStrategy:</p><p>            inline:</p><p>              shardingColumn: user_id</p><p>              algorithmExpression: ms_ds${0}</p><p>          tableStrategy: </p><p>            inline:</p><p>              shardingColumn: id</p><p>              algorithmExpression: t_user_${id % 2}</p><p>          keyGenerator:</p><p>            type: SNOWFLAKE</p><p>            column: id</p><p>      bindingTables:</p><p>        - t_user</p><p>      broadcastTables:</p><p>        - t_config</p><p>      </p><p>      defaultDataSourceName: ds0</p><p>      defaultTableStrategy:</p><p>        none:</p><p>      </p><p>      masterSlaveRules:</p><p>        ms_ds0:</p><p>          masterDataSourceName: ds0</p><p>          slaveDataSourceNames:</p><p>            - ds0_slave0</p><p>    </p><p>    </p><p><br></p><p>![1612272530622](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039211.png)</p><p><br></p><p>![1612272899041](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039218.png)</p><p><br></p><p>**9.Sharding-Sidecar（TODO）**</p><p>----------------------------</p><p><br></p><p>\\*\\*简介：\\*\\*定位为Kubernetes的云原生数据库代理，以Sidecar的形式代理所有对数据库的访问。 通过无中心、零侵入的方案提供与数据库交互的的啮合层，即Database Mesh，又可称数据网格。</p><p><br></p><p>Database Mesh的关注重点在于如何将分布式的数据访问应用与数据库有机串联起来，它更加关注的是交互，是将杂乱无章的应用与数据库之间的交互有效的梳理。使用Database Mesh，访问数据库的应用和数据库终将形成一个巨大的网格体系，应用和数据库只需在网格体系中对号入座即可，它们都是被啮合层所治理的对象。</p><p><br></p><p>![1612270968937](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039189.png)</p><p><br></p><p>_Sharding-JDBC_</p><p><br></p><p>_Sharding-Proxy_</p><p><br></p><p>_Sharding-Sidecar_</p><p><br></p><p>&nbsp;</p><p><br></p><p>数据库</p><p><br></p><p>任意</p><p><br></p><p>MySQL</p><p><br></p><p>MySQL</p><p><br></p><p>连接消耗数</p><p><br></p><p>高</p><p><br></p><p>低</p><p><br></p><p>高</p><p><br></p><p>异构语言</p><p><br></p><p>仅Java</p><p><br></p><p>任意</p><p><br></p><p>任意</p><p><br></p><p>性能</p><p><br></p><p>损耗低</p><p><br></p><p>损耗略高</p><p><br></p><p>损耗低</p><p><br></p><p>无中心化</p><p><br></p><p>是</p><p><br></p><p>否</p><p><br></p><p>是</p><p><br></p><p>静态入口</p><p><br></p><p>无</p><p><br></p><p>有</p><p><br></p><p>无</p><p><br></p><p>**混合架构**</p><p><br></p><p>Sharding-JDBC采用无中心化架构，适用于Java开发的高性能的轻量级OLTP应用；Sharding-Proxy提供静态入口以及异构语言的支持，适用于OLAP应用以及对分片数据库进行管理和运维的场景。</p><p><br></p><p>ShardingSphere是多接入端共同组成的生态圈。 通过混合使用Sharding-JDBC和Sharding-Proxy，并采用同一注册中心统一配置分片策略，能够灵活的搭建适用于各种场景的应用系统，架构师可以更加自由的调整适合于当前业务的最佳系统架构。</p><p><br></p><p>![1612271069036](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039197.png)</p><p><br></p><p>10.为什么选**ShardingSphere**作为分库分表中间件而不选**Mycat**</p><p>----------------------------------------------</p><p><br></p><p>**相同点：shadingsphere**和**mycat**都是为了解决**分库分表的中间件**。都是优秀的开源的项目。mycat开源较早。mycat在。ShardingSphere前身是**当当网**的。ShardingSphere 已于2020年4月16日成为 Apache 软件基金会的顶级项目。让ShardingSphere更上一层楼。**mycat**是通过配置 将数据库逻辑表与实体表在配置文件中对应，由mycat处理逻辑表与实体表的实际操作，应用程序层连接mycat，跟连接mysql数据库一模一样，应用程序操作数据库的逻辑表，逻辑表通过配置文件对应到实体的表。 应用程序层不做处理，mycat是安装在服务器端，在服务器端再进行的实际sql的执行。实际sql的执行也同**shardingspere**一样，**解析、改写、路由、结果归集**等操作。</p><p><br></p><p>**MYCAT**</p><p><br></p><p>**2013**年阿里的Cobar在社区使用过程中发现存在一些比较严重的问题，及其使用限制，经过Mycat发起人第一次改良，第一代改良版——Mycat诞生。 Mycat开源以后，一些Cobar的用户参与了Mycat的开发，最终Mycat发展成为一个由众多软件公司的实力派架构师和资深开发人员维护的社区型开源软件。**2014**年Mycat首次在上海的《中华架构师》大会上对外宣讲，更多的人参与进来，随后越来越多的项目采用了Mycat。**2015年5月**，由核心参与者们一起编写的第一本官方权威指南《Mycat权威指南》电子版发布，累计超过500本，成为开源项目中的首创。**2015年10月**为止，Mycat项目总共有16个Committer。截至**2015年11月**，超过300个项目采用Mycat，涵盖银行、电信、电子商务、物流、移动应用、O2O的众多领域和公司。</p><p><br></p><p>Mycat的官网。我在网上搜的时候实际上很难搜到这个网站。我进去后我怀疑我搞错了。因为MYCAT在我看来是非常强大的中间件。</p><p><br></p><p>![1612273777435](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039225.png)</p><p><br></p><p>查看**代码更新**的时间 实际上还在**持续**的更新当中。</p><p><br></p><p>![1612273911109](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039231.png)</p><p><br></p><p>**ShardingShpere**</p><p><br></p><p>**官方网站**：[https://shardingsphere.apache.org/index\\_zh.html](https://shardingsphere.apache.org/index_zh.html)</p><p><br></p><p>![1612274073435](http://image.ithhit.cn/blog/admin/png/2021/2/2/1612275039245.png)</p><p><br></p><p>不管是从文档还是官方文档方方面面都做的特别好。还称为了apache基金会的顶级项目。对分库分表做了一系列的解决方案。版本还在持续更新中。并且更新的速度很快。从去年的2020年4月份3.0版本 到目前的5.X版本。</p><p><br></p><p>实际上两个中间件。不过我更偏向ShardingShpere。</p>', '2,3,4', 0, 0, '/common/image/2021/08/29/67b957b05e204c0baf8ac5564e2f5f15/67b957b05e204c0baf8ac5564e2f5f15.jpeg', 1, NULL, '1', '2112212121', NULL, '2', 2, NULL, '0', 1, 1, 0, NULL, NULL, 0, NULL, 1, '211221', 'admin', '2021-08-28 18:08:10', '2021-08-29 11:43:51', 'admin', '211221');
INSERT INTO `blog_article` VALUES (5, '111111', '1111', '<p>1111111</p>', '2,3,4', 0, 0, '/common/image/2021/08/29/3a7b9fa7225c430d94459cef263d8edc/3a7b9fa7225c430d94459cef263d8edc.png', 1, NULL, '1', '1111', NULL, '2', 0, NULL, '1', 0, 1, 0, NULL, NULL, 0, NULL, 1, '111111111111', 'admin', '2021-08-29 10:19:45', '2021-08-29 11:46:17', 'admin', '11111111111');
INSERT INTO `blog_article` VALUES (6, '3232', '323232', '<p>32323232</p>', '2,3,4', 0, 0, NULL, 1, NULL, '1', NULL, NULL, '2', 0, NULL, '1', 7, 1, 0, NULL, NULL, 0, NULL, 1, '323232', 'admin', '2021-08-29 11:49:28', '2021-08-29 11:49:28', 'admin', '332322332');
...
CREATE TABLE `blog_article_collect`  (
  `article_collect_id` bigint(32) NOT NULL AUTO_INCREMENT COMMENT '主键',
  ...
  `artcile_id` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '博客的id',
  ...
  `update_by` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '更新人',
  `remark` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`article_collect_id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 2 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '文章收藏表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `blog_sort`  (
  `sort_id` bigint(32) NOT NULL AUTO_INCREMENT COMMENT '唯一id',
...
-- ----------------------------
INSERT INTO `blog_sort` VALUES (2, '121221', '<p>122121211212</p>', 1, 0, 0, 'admin', '2021-08-28 17:17:10', 'admin', '2021-08-28 17:40:10', '21212121');
INSERT INTO `blog_sort` VALUES (3, '123122121', '<p>1232121321</p>', 0, 1, 0, 'admin', '2021-08-28 17:20:19', 'admin', '2021-08-28 17:40:23', NULL);
INSERT INTO `blog_sort` VALUES (4, '23323232', '<p>233232</p>', 1, 0, 0, 'admin', '2021-08-29 10:20:15', 'admin', '2021-08-29 10:20:14', '233232');
...
-- Records of blog_tag
-- ----------------------------
INSERT INTO `blog_tag` VALUES (2, '学习笔记', 1, 0, 0, '2021-08-28 17:16:58', '2021-08-28 18:19:18', 'admin', 'admin', NULL);
INSERT INTO `blog_tag` VALUES (3, '摩西摩西', 1, 0, 0, '2021-08-28 18:21:33', '2021-08-28 18:21:32', 'admin', 'admin', '1');
INSERT INTO `blog_tag` VALUES (4, '32323223', 1, 0, 0, '2021-08-29 10:20:07', '2021-08-29 10:20:06', 'admin', 'admin', '3232');
...
INSERT INTO `gen_table` VALUES (10, 'blog_article_collect', '文章收藏表', NULL, NULL, 'ArticleCollect', 'crud', 'com.ruoyi.blog', 'blog', 'collect', '文章收藏', 'thh', '0', '/', '{\"parentMenuId\":\"2036\"}', 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26', NULL);
...
) ENGINE = InnoDB AUTO_INCREMENT = 192 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '代码生成业务表字段' ROW_FORMAT = Dynamic;
...
INSERT INTO `gen_table_column` VALUES (137, '10', 'article_collect_id', '主键', 'bigint(32)', 'Long', 'articleCollectId', '1', '1', NULL, '1', NULL, NULL, NULL, 'EQ', 'input', '', 1, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (138, '10', 'user_id', '用户的id', 'varchar(32)', 'String', 'userId', '0', '0', '1', '1', '1', '1', NULL, 'EQ', 'input', '', 2, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (139, '10', 'user_ip', '用户ip', 'varchar(255)', 'String', 'userIp', '0', '0', NULL, '1', '1', '1', NULL, 'EQ', 'input', '', 3, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (140, '10', 'artcile_id', '博客的id', 'varchar(32)', 'String', 'artcileId', '0', '0', '1', '1', '1', '1', NULL, 'EQ', 'input', '', 4, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (141, '10', 'status', '状态', 'tinyint(1) unsigned', 'Integer', 'status', '0', '0', '1', '1', '1', '1', '1', 'EQ', 'radio', 'article_status', 5, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (142, '10', 'create_time', '创建时间', 'timestamp', 'Date', 'createTime', '0', '0', '1', NULL, NULL, NULL, NULL, 'EQ', 'datetime', '', 6, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (143, '10', 'update_time', '更新时间', 'timestamp', 'Date', 'updateTime', '0', '0', '1', NULL, NULL, NULL, NULL, 'EQ', 'datetime', '', 7, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (144, '10', 'create_by', '创建人', 'varchar(64)', 'String', 'createBy', '0', '0', NULL, NULL, NULL, NULL, NULL, 'EQ', 'input', '', 8, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (145, '10', 'update_by', '更新人', 'varchar(64)', 'String', 'updateBy', '0', '0', NULL, NULL, NULL, NULL, NULL, 'EQ', 'input', '', 9, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
INSERT INTO `gen_table_column` VALUES (146, '10', 'remark', '备注', 'varchar(255)', 'String', 'remark', '0', '0', NULL, '1', '1', NULL, NULL, 'EQ', 'input', '', 10, 'admin', '2021-08-28 13:05:41', '', '2021-08-28 15:30:26');
...
INSERT INTO `qiwen_file` VALUES (140, 3761458, '/upload/20210620/17c044003ad08706238a5282682c5028.zip', '17c044003ad08706238a5282682c5028', 1, 0, '17c044003ad08706238a5282682c5028');
INSERT INTO `qiwen_file` VALUES (143, 3820291, '\\upload\\20210620\\17c044003ad08706238a5282682c5028\\3423232\\《辐射4》（Fallout 4）4k桌面壁纸_彼岸图网.jpg', NULL, 1, 0, '《辐射4》（Fallout 4）4k桌面壁纸_彼岸图网');
INSERT INTO `qiwen_file` VALUES (152, 2401524, '/upload/20210620/dc099106d3ad3bff20ddc13ce390671a.jpg', 'dc099106d3ad3bff20ddc13ce390671a', 2, 0, 'dc099106d3ad3bff20ddc13ce390671a');
INSERT INTO `qiwen_file` VALUES (153, 148267788, '/upload/20210703/2fbb52a8eba53c73a47338d3abfd773a.mp4', '2fbb52a8eba53c73a47338d3abfd773a', 1, 0, '2fbb52a8eba53c73a47338d3abfd773a');
INSERT INTO `qiwen_file` VALUES (154, 149969669, '/upload/20210703/8e1d17817adae490848bf1b0637ad661.mp4', '8e1d17817adae490848bf1b0637ad661', 1, 0, '8e1d17817adae490848bf1b0637ad661');
INSERT INTO `qiwen_file` VALUES (155, 832992, '/upload/20210707/179a0ffb1ec170fa6e4fce31eddc75a6.docx', '179a0ffb1ec170fa6e4fce31eddc75a6', 1, 0, '179a0ffb1ec170fa6e4fce31eddc75a6');
INSERT INTO `qiwen_file` VALUES (156, 60552192, '/upload/20210707/6c1fb361f9645141333313e94d8aae17.avi', '6c1fb361f9645141333313e94d8aae17', 1, 0, '6c1fb361f9645141333313e94d8aae17');
INSERT INTO `qiwen_file` VALUES (157, 108553641, '/upload/20210707/a216390a9443d4b602e780cdd7456a29.mp4', 'a216390a9443d4b602e780cdd7456a29', 1, 0, 'a216390a9443d4b602e780cdd7456a29');
INSERT INTO `qiwen_file` VALUES (158, 71282, '/upload/20210814/6418ea6dcd8baab68fe9b58fcfe2057c.jpg', '6418ea6dcd8baab68fe9b58fcfe2057c', 1, 0, '6418ea6dcd8baab68fe9b58fcfe2057c');
...
INSERT INTO `qiwen_file` VALUES (163, 53385065, '/upload/20210821/f179d02fe67e0f38c273f04d5e888fee.mp4', 'f179d02fe67e0f38c273f04d5e888fee', 1, 0, 'f179d02fe67e0f38c273f04d5e888fee');
...
-- ----------------------------
INSERT INTO `qiwen_recovery_file` VALUES (243, '9f036337-9bd2-486e-9fc6-0f0619d903ae', '2021-06-20 11:01:32', 254);
...
INSERT INTO `qiwen_user_file` VALUES (235, NULL, 0, NULL, 'zip', 140, '3423232', '/', 0, '2021-06-20 10:30:59', 1);
INSERT INTO `qiwen_user_file` VALUES (236, NULL, 0, NULL, NULL, NULL, '3423232', '/', 1, '2021-06-20 10:31:15', 1);
...
INSERT INTO `qiwen_user_file` VALUES (242, NULL, 0, NULL, NULL, NULL, '3423232', '/', 1, '2021-06-20 10:34:39', 1);
INSERT INTO `qiwen_user_file` VALUES (243, NULL, 0, NULL, 'jpg', 143, '《辐射4》（Fallout 4）4k桌面壁纸_彼岸图网', '/3423232/', 0, '2021-06-20 10:34:39', 1);
...
INSERT INTO `qiwen_user_file` VALUES (254, '9f036337-9bd2-486e-9fc6-0f0619d903ae', 655712, '2021-06-20 11:01:32', 'jpg', 152, '《Control》4k游戏壁纸3840x2160_彼岸图网', '/', 0, '2021-06-20 11:01:16', 1);
INSERT INTO `qiwen_user_file` VALUES (255, NULL, NULL, NULL, 'jpg', 152, '《Control》4k游戏壁纸3840x2160_彼岸图网', '/', 0, '2021-06-20 11:01:41', 1);
INSERT INTO `qiwen_user_file` VALUES (256, NULL, 0, NULL, 'mp4', 153, '09_支付模块构建(中)1', '/', 0, '2021-07-03 15:02:06', 1);
INSERT INTO `qiwen_user_file` VALUES (257, NULL, 0, NULL, 'mp4', 154, '148_Seata之原理简介', '/', 0, '2021-07-03 16:05:26', 1);
INSERT INTO `qiwen_user_file` VALUES (258, NULL, 0, NULL, 'docx', 155, '10.南方电网公司电网管理平台-安全生产-操作票管理应用概要设计V1.0(2)(1)', '/', 0, '2021-07-07 19:58:45', 1);
INSERT INTO `qiwen_user_file` VALUES (259, NULL, 0, NULL, 'avi', 156, '5.RabbitMQ研究-入门程序-生产者', '/', 0, '2021-07-07 20:36:52', 1);
INSERT INTO `qiwen_user_file` VALUES (260, NULL, 0, NULL, 'mp4', 157, '爱剪辑-Cleaning', '/', 0, '2021-07-07 20:38:42', 1);
INSERT INTO `qiwen_user_file` VALUES (261, NULL, 0, NULL, 'jpg', 158, 'THH', '/', 0, '2021-08-14 19:06:38', 1);
...
INSERT INTO `qiwen_user_file` VALUES (266, NULL, 0, NULL, 'mp4', 163, '99_Nacos之服务消费者注册和负载', '/', 0, '2021-08-21 11:19:34', 1);
...
INSERT INTO `qrtz_job_details` VALUES ('RuoyiScheduler', 'TASK_CLASS_NAME4', 'DEFAULT', NULL, 'com.ruoyi.quartz.util.QuartzDisallowConcurrentExecution', '0', '1', '0', '0', );
...
-- ----------------------------
INSERT INTO `qrtz_scheduler_state` VALUES ('RuoyiScheduler', 'DESKTOP-MJHPK561630199313771', 1630210708132, 15000);
...
-- ----------------------------
INSERT INTO `qrtz_triggers` VALUES ('RuoyiScheduler', 'TASK_CLASS_NAME1', 'DEFAULT', 'TASK_CLASS_NAME1', 'DEFAULT', NULL, 1630199320000, -1, 5, 'PAUSED', 'CRON', 1630199314000, 0, NULL, 2, '');
INSERT INTO `qrtz_triggers` VALUES ('RuoyiScheduler', 'TASK_CLASS_NAME2', 'DEFAULT', 'TASK_CLASS_NAME2', 'DEFAULT', NULL, 1630199325000, -1, 5, 'PAUSED', 'CRON', 1630199314000, 0, NULL, 2, '');
INSERT INTO `qrtz_triggers` VALUES ('RuoyiScheduler', 'TASK_CLASS_NAME3', 'DEFAULT', 'TASK_CLASS_NAME3', 'DEFAULT', NULL, 1630199320000, -1, 5, 'PAUSED', 'CRON', 1630199315000, 0, NULL, 2, '');
INSERT INTO `qrtz_triggers` VALUES ('RuoyiScheduler', 'TASK_CLASS_NAME4', 'DEFAULT', 'TASK_CLASS_NAME4', 'DEFAULT', NULL, 1630210739000, 1630210680000, 5, 'WAITING', 'CRON', 1630199315000, 0, NULL, -1, '');
...
) ENGINE = InnoDB AUTO_INCREMENT = 176 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '字典数据表' ROW_FORMAT = Dynamic;
...
INSERT INTO `sys_dict_data` VALUES (140, 0, '天龙八部', '天龙八部', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:05:51', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (141, 0, '大宋', '大宋', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:06', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (142, 0, '北宋', '北宋', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:16', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (143, 0, '年间', '年间', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:23', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (144, 0, '国土', '国土', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:31', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (145, 0, '悍匪', '悍匪', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:40', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (146, 0, '集团', '集团', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:06:46', 'admin', '2021-08-24 20:06:54', NULL);
INSERT INTO `sys_dict_data` VALUES (147, 0, '刑警', '刑警', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:00', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (148, 0, ' 追捕', ' 追捕', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:09', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (149, 0, '唐三', '唐三', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:26', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (150, 0, '斗罗大陆', '斗罗大陆', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:36', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (151, 0, '为唐', '为唐', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:43', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (152, 0, '绝学', '绝学', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:07:50', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (153, 0, '网游', '网游', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:03', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (154, 0, '游戏', '游戏', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:10', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (155, 0, '荣耀', '荣耀', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:19', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (156, 0, ' 天下无贼', ' 天下无贼', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:36', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (157, 0, '城市', '城市', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:46', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (158, 0, '动漫版', '动漫版', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:08:57', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (159, 0, '艰难险阻', '艰难险阻', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:09:11', '', NULL, NULL);
INSERT INTO `sys_dict_data` VALUES (160, 0, '金融中心', '金融中心', 'movie_label', NULL, 'default', 'N', '0', 'admin', '2021-08-24 20:09:33', '', NULL, NULL);
...
INSERT INTO `sys_job` VALUES (4, '转换流媒体文件', 'DEFAULT', 'mediaTask.convertVideoList', '0/59 * * * * ?', '1', '1', '0', 'admin', '2021-07-05 20:36:46', 'admin', '2021-07-05 21:33:05', '');
...
) ENGINE = InnoDB AUTO_INCREMENT = 8023 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '定时任务调度日志表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of sys_job_log
-- ----------------------------
INSERT INTO `sys_job_log` VALUES (8021, '转换流媒体文件', 'DEFAULT', 'mediaTask.convertVideoList', '转换流媒体文件 总共耗时：1毫秒', '0', '', '2021-08-29 12:17:59');
INSERT INTO `sys_job_log` VALUES (8022, '转换流媒体文件', 'DEFAULT', 'mediaTask.convertVideoList', '转换流媒体文件 总共耗时：2毫秒', '0', '', '2021-08-29 12:18:00');
...
) ENGINE = InnoDB AUTO_INCREMENT = 752 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '系统访问记录' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of sys_logininfor
-- ----------------------------
INSERT INTO `sys_logininfor` VALUES (203, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-05-16 09:44:03');
INSERT INTO `sys_logininfor` VALUES (204, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-16 10:04:02');
INSERT INTO `sys_logininfor` VALUES (205, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-16 10:57:18');
INSERT INTO `sys_logininfor` VALUES (206, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-16 13:22:17');
INSERT INTO `sys_logininfor` VALUES (207, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-16 14:50:12');
INSERT INTO `sys_logininfor` VALUES (208, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-19 20:28:49');
INSERT INTO `sys_logininfor` VALUES (209, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-19 20:28:58');
INSERT INTO `sys_logininfor` VALUES (210, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-19 22:05:22');
INSERT INTO `sys_logininfor` VALUES (211, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-20 20:47:43');
INSERT INTO `sys_logininfor` VALUES (212, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-21 21:13:46');
INSERT INTO `sys_logininfor` VALUES (213, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 08:39:22');
INSERT INTO `sys_logininfor` VALUES (214, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 09:13:58');
INSERT INTO `sys_logininfor` VALUES (215, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 09:50:31');
INSERT INTO `sys_logininfor` VALUES (216, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 10:23:52');
INSERT INTO `sys_logininfor` VALUES (217, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 12:54:39');
INSERT INTO `sys_logininfor` VALUES (218, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-22 16:58:05');
INSERT INTO `sys_logininfor` VALUES (219, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-23 16:34:54');
INSERT INTO `sys_logininfor` VALUES (220, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-23 16:35:26');
INSERT INTO `sys_logininfor` VALUES (221, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-24 20:28:55');
INSERT INTO `sys_logininfor` VALUES (222, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-24 23:23:10');
INSERT INTO `sys_logininfor` VALUES (223, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-24 23:33:31');
INSERT INTO `sys_logininfor` VALUES (224, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-24 23:45:18');
INSERT INTO `sys_logininfor` VALUES (225, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-24 23:50:39');
INSERT INTO `sys_logininfor` VALUES (226, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 00:06:18');
INSERT INTO `sys_logininfor` VALUES (227, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 00:09:25');
INSERT INTO `sys_logininfor` VALUES (228, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 00:39:34');
INSERT INTO `sys_logininfor` VALUES (229, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 19:58:46');
INSERT INTO `sys_logininfor` VALUES (230, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 20:49:38');
INSERT INTO `sys_logininfor` VALUES (231, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-25 20:57:54');
INSERT INTO `sys_logininfor` VALUES (232, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 21:10:04');
INSERT INTO `sys_logininfor` VALUES (233, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-05-26 22:11:23');
INSERT INTO `sys_logininfor` VALUES (234, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:26:54');
INSERT INTO `sys_logininfor` VALUES (235, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:34:51');
INSERT INTO `sys_logininfor` VALUES (236, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:37:16');
INSERT INTO `sys_logininfor` VALUES (237, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:38:19');
INSERT INTO `sys_logininfor` VALUES (238, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:39:41');
INSERT INTO `sys_logininfor` VALUES (239, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 22:41:48');
INSERT INTO `sys_logininfor` VALUES (240, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 23:02:26');
INSERT INTO `sys_logininfor` VALUES (241, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 23:05:54');
INSERT INTO `sys_logininfor` VALUES (242, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 23:40:58');
INSERT INTO `sys_logininfor` VALUES (243, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 23:56:50');
INSERT INTO `sys_logininfor` VALUES (244, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-26 23:59:11');
INSERT INTO `sys_logininfor` VALUES (245, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:00:37');
INSERT INTO `sys_logininfor` VALUES (246, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:01:22');
INSERT INTO `sys_logininfor` VALUES (247, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:02:26');
INSERT INTO `sys_logininfor` VALUES (248, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:02:55');
INSERT INTO `sys_logininfor` VALUES (249, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:04:33');
INSERT INTO `sys_logininfor` VALUES (250, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:05:46');
INSERT INTO `sys_logininfor` VALUES (251, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:22:57');
INSERT INTO `sys_logininfor` VALUES (252, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:23:50');
INSERT INTO `sys_logininfor` VALUES (253, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:50:50');
INSERT INTO `sys_logininfor` VALUES (254, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 00:57:38');
INSERT INTO `sys_logininfor` VALUES (255, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 01:01:30');
INSERT INTO `sys_logininfor` VALUES (256, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 01:15:07');
INSERT INTO `sys_logininfor` VALUES (257, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 02:09:04');
INSERT INTO `sys_logininfor` VALUES (258, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 02:39:30');
INSERT INTO `sys_logininfor` VALUES (259, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 08:49:13');
INSERT INTO `sys_logininfor` VALUES (260, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 09:18:14');
INSERT INTO `sys_logininfor` VALUES (261, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 10:19:22');
INSERT INTO `sys_logininfor` VALUES (262, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 10:23:10');
INSERT INTO `sys_logininfor` VALUES (263, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 15:06:13');
INSERT INTO `sys_logininfor` VALUES (264, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 15:10:01');
INSERT INTO `sys_logininfor` VALUES (265, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 15:16:03');
INSERT INTO `sys_logininfor` VALUES (266, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 16:04:54');
INSERT INTO `sys_logininfor` VALUES (267, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 16:05:57');
INSERT INTO `sys_logininfor` VALUES (268, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 16:40:58');
INSERT INTO `sys_logininfor` VALUES (269, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 17:33:32');
INSERT INTO `sys_logininfor` VALUES (270, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 17:52:24');
INSERT INTO `sys_logininfor` VALUES (271, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 17:53:59');
INSERT INTO `sys_logininfor` VALUES (272, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 17:56:56');
INSERT INTO `sys_logininfor` VALUES (273, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 17:57:08');
INSERT INTO `sys_logininfor` VALUES (274, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 17:59:03');
INSERT INTO `sys_logininfor` VALUES (275, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 17:59:07');
INSERT INTO `sys_logininfor` VALUES (276, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 17:59:30');
INSERT INTO `sys_logininfor` VALUES (277, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-27 17:59:30');
INSERT INTO `sys_logininfor` VALUES (278, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 17:59:35');
INSERT INTO `sys_logininfor` VALUES (279, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:01:03');
INSERT INTO `sys_logininfor` VALUES (280, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:01:06');
INSERT INTO `sys_logininfor` VALUES (281, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:02:46');
INSERT INTO `sys_logininfor` VALUES (282, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:05:35');
INSERT INTO `sys_logininfor` VALUES (283, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:05:41');
INSERT INTO `sys_logininfor` VALUES (284, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 18:06:45');
INSERT INTO `sys_logininfor` VALUES (285, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-27 18:06:45');
INSERT INTO `sys_logininfor` VALUES (286, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-27 18:06:47');
INSERT INTO `sys_logininfor` VALUES (287, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:06:55');
INSERT INTO `sys_logininfor` VALUES (288, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:06:57');
INSERT INTO `sys_logininfor` VALUES (289, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:07:05');
INSERT INTO `sys_logininfor` VALUES (290, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:10:02');
INSERT INTO `sys_logininfor` VALUES (291, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:10:08');
INSERT INTO `sys_logininfor` VALUES (292, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-27 18:10:16');
INSERT INTO `sys_logininfor` VALUES (293, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-27 18:10:21');
INSERT INTO `sys_logininfor` VALUES (294, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:10:28');
INSERT INTO `sys_logininfor` VALUES (295, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:11:10');
INSERT INTO `sys_logininfor` VALUES (296, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:21:41');
INSERT INTO `sys_logininfor` VALUES (297, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:23:00');
INSERT INTO `sys_logininfor` VALUES (298, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 18:23:03');
INSERT INTO `sys_logininfor` VALUES (299, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:24:45');
INSERT INTO `sys_logininfor` VALUES (300, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:27:21');
INSERT INTO `sys_logininfor` VALUES (301, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 18:40:36');
INSERT INTO `sys_logininfor` VALUES (302, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:00:47');
INSERT INTO `sys_logininfor` VALUES (303, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:28:59');
INSERT INTO `sys_logininfor` VALUES (304, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:29:05');
INSERT INTO `sys_logininfor` VALUES (305, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:33:40');
INSERT INTO `sys_logininfor` VALUES (306, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:33:45');
INSERT INTO `sys_logininfor` VALUES (307, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 19:36:58');
INSERT INTO `sys_logininfor` VALUES (308, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:37:05');
INSERT INTO `sys_logininfor` VALUES (309, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:37:14');
INSERT INTO `sys_logininfor` VALUES (310, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:52:43');
INSERT INTO `sys_logininfor` VALUES (311, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:52:45');
INSERT INTO `sys_logininfor` VALUES (312, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:53:27');
INSERT INTO `sys_logininfor` VALUES (313, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:55:08');
INSERT INTO `sys_logininfor` VALUES (314, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:56:28');
INSERT INTO `sys_logininfor` VALUES (315, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:57:20');
INSERT INTO `sys_logininfor` VALUES (316, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 19:57:24');
INSERT INTO `sys_logininfor` VALUES (317, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 19:57:31');
INSERT INTO `sys_logininfor` VALUES (318, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:00:23');
INSERT INTO `sys_logininfor` VALUES (319, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:00:25');
INSERT INTO `sys_logininfor` VALUES (320, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:01:36');
INSERT INTO `sys_logininfor` VALUES (321, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:01:38');
INSERT INTO `sys_logininfor` VALUES (322, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:02:04');
INSERT INTO `sys_logininfor` VALUES (323, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:02:06');
INSERT INTO `sys_logininfor` VALUES (324, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:02:51');
INSERT INTO `sys_logininfor` VALUES (325, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:02:56');
INSERT INTO `sys_logininfor` VALUES (326, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:06:09');
INSERT INTO `sys_logininfor` VALUES (327, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:06:10');
INSERT INTO `sys_logininfor` VALUES (328, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:06:40');
INSERT INTO `sys_logininfor` VALUES (329, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:06:42');
INSERT INTO `sys_logininfor` VALUES (330, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:08:00');
INSERT INTO `sys_logininfor` VALUES (331, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 20:08:02');
INSERT INTO `sys_logininfor` VALUES (332, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:08:06');
INSERT INTO `sys_logininfor` VALUES (333, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:09:21');
INSERT INTO `sys_logininfor` VALUES (334, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:09:26');
INSERT INTO `sys_logininfor` VALUES (335, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:09:43');
INSERT INTO `sys_logininfor` VALUES (336, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:09:45');
INSERT INTO `sys_logininfor` VALUES (337, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 20:36:38');
INSERT INTO `sys_logininfor` VALUES (338, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:36:42');
INSERT INTO `sys_logininfor` VALUES (339, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:49:57');
INSERT INTO `sys_logininfor` VALUES (340, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:52:01');
INSERT INTO `sys_logininfor` VALUES (341, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:52:36');
INSERT INTO `sys_logininfor` VALUES (342, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:52:45');
INSERT INTO `sys_logininfor` VALUES (343, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:53:19');
INSERT INTO `sys_logininfor` VALUES (344, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 20:53:24');
INSERT INTO `sys_logininfor` VALUES (345, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:53:30');
INSERT INTO `sys_logininfor` VALUES (346, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 20:57:55');
INSERT INTO `sys_logininfor` VALUES (347, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:57:59');
INSERT INTO `sys_logininfor` VALUES (348, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:58:19');
INSERT INTO `sys_logininfor` VALUES (349, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 20:58:49');
INSERT INTO `sys_logininfor` VALUES (350, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:00:53');
INSERT INTO `sys_logininfor` VALUES (351, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:03:46');
INSERT INTO `sys_logininfor` VALUES (352, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 21:04:52');
INSERT INTO `sys_logininfor` VALUES (353, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:04:57');
INSERT INTO `sys_logininfor` VALUES (354, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:08:01');
INSERT INTO `sys_logininfor` VALUES (355, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 21:08:08');
INSERT INTO `sys_logininfor` VALUES (356, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:10:07');
INSERT INTO `sys_logininfor` VALUES (357, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:12:00');
INSERT INTO `sys_logininfor` VALUES (358, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 21:12:18');
INSERT INTO `sys_logininfor` VALUES (359, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:13:00');
INSERT INTO `sys_logininfor` VALUES (360, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 21:13:38');
INSERT INTO `sys_logininfor` VALUES (361, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:14:31');
INSERT INTO `sys_logininfor` VALUES (362, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:15:08');
INSERT INTO `sys_logininfor` VALUES (363, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:16:38');
INSERT INTO `sys_logininfor` VALUES (364, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:17:47');
INSERT INTO `sys_logininfor` VALUES (365, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:18:25');
INSERT INTO `sys_logininfor` VALUES (366, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:19:19');
INSERT INTO `sys_logininfor` VALUES (367, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:20:52');
INSERT INTO `sys_logininfor` VALUES (368, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:21:12');
INSERT INTO `sys_logininfor` VALUES (369, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:41:57');
INSERT INTO `sys_logininfor` VALUES (370, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:42:18');
INSERT INTO `sys_logininfor` VALUES (371, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:43:49');
INSERT INTO `sys_logininfor` VALUES (372, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:43:53');
INSERT INTO `sys_logininfor` VALUES (373, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:44:01');
INSERT INTO `sys_logininfor` VALUES (374, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:44:14');
INSERT INTO `sys_logininfor` VALUES (375, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 21:44:55');
INSERT INTO `sys_logininfor` VALUES (376, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 22:14:00');
INSERT INTO `sys_logininfor` VALUES (377, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 22:14:55');
INSERT INTO `sys_logininfor` VALUES (378, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 22:15:13');
INSERT INTO `sys_logininfor` VALUES (379, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 22:58:54');
INSERT INTO `sys_logininfor` VALUES (380, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:13:35');
INSERT INTO `sys_logininfor` VALUES (381, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:14:56');
INSERT INTO `sys_logininfor` VALUES (382, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:15:32');
INSERT INTO `sys_logininfor` VALUES (383, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:21:46');
INSERT INTO `sys_logininfor` VALUES (384, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 23:24:50');
INSERT INTO `sys_logininfor` VALUES (385, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:24:53');
INSERT INTO `sys_logininfor` VALUES (386, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:25:36');
INSERT INTO `sys_logininfor` VALUES (387, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:26:52');
INSERT INTO `sys_logininfor` VALUES (388, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:27:06');
INSERT INTO `sys_logininfor` VALUES (389, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:28:18');
INSERT INTO `sys_logininfor` VALUES (390, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-27 23:28:54');
INSERT INTO `sys_logininfor` VALUES (391, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:30:18');
INSERT INTO `sys_logininfor` VALUES (392, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:30:31');
INSERT INTO `sys_logininfor` VALUES (393, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:31:26');
INSERT INTO `sys_logininfor` VALUES (394, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:32:32');
INSERT INTO `sys_logininfor` VALUES (395, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:33:29');
INSERT INTO `sys_logininfor` VALUES (396, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:34:17');
INSERT INTO `sys_logininfor` VALUES (397, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:34:52');
INSERT INTO `sys_logininfor` VALUES (398, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:40:35');
INSERT INTO `sys_logininfor` VALUES (399, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:40:54');
INSERT INTO `sys_logininfor` VALUES (400, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:41:04');
INSERT INTO `sys_logininfor` VALUES (401, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:44:45');
INSERT INTO `sys_logininfor` VALUES (402, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:45:19');
INSERT INTO `sys_logininfor` VALUES (403, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 23:48:29');
INSERT INTO `sys_logininfor` VALUES (404, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:48:31');
INSERT INTO `sys_logininfor` VALUES (405, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:49:00');
INSERT INTO `sys_logininfor` VALUES (406, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:49:09');
INSERT INTO `sys_logininfor` VALUES (407, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-27 23:49:27');
INSERT INTO `sys_logininfor` VALUES (408, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:49:31');
INSERT INTO `sys_logininfor` VALUES (409, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:51:05');
INSERT INTO `sys_logininfor` VALUES (410, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:53:31');
INSERT INTO `sys_logininfor` VALUES (411, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:55:53');
INSERT INTO `sys_logininfor` VALUES (412, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:56:11');
INSERT INTO `sys_logininfor` VALUES (413, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:58:49');
INSERT INTO `sys_logininfor` VALUES (414, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-27 23:59:11');
INSERT INTO `sys_logininfor` VALUES (415, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:01:11');
INSERT INTO `sys_logininfor` VALUES (416, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:04:21');
INSERT INTO `sys_logininfor` VALUES (417, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:05:05');
INSERT INTO `sys_logininfor` VALUES (418, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:06:04');
INSERT INTO `sys_logininfor` VALUES (419, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-28 00:07:20');
INSERT INTO `sys_logininfor` VALUES (420, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:07:23');
INSERT INTO `sys_logininfor` VALUES (421, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:12:56');
INSERT INTO `sys_logininfor` VALUES (422, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:13:09');
INSERT INTO `sys_logininfor` VALUES (423, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:14:36');
INSERT INTO `sys_logininfor` VALUES (424, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 00:16:10');
INSERT INTO `sys_logininfor` VALUES (425, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 19:40:31');
INSERT INTO `sys_logininfor` VALUES (426, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 21:50:53');
INSERT INTO `sys_logininfor` VALUES (427, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 22:08:23');
INSERT INTO `sys_logininfor` VALUES (428, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 22:28:23');
INSERT INTO `sys_logininfor` VALUES (429, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 22:31:41');
INSERT INTO `sys_logininfor` VALUES (430, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 22:33:20');
INSERT INTO `sys_logininfor` VALUES (431, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 22:54:50');
INSERT INTO `sys_logininfor` VALUES (432, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:01:44');
INSERT INTO `sys_logininfor` VALUES (433, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-28 23:02:10');
INSERT INTO `sys_logininfor` VALUES (434, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:07:38');
INSERT INTO `sys_logininfor` VALUES (435, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:07:55');
INSERT INTO `sys_logininfor` VALUES (436, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:08:10');
INSERT INTO `sys_logininfor` VALUES (437, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-28 23:08:20');
INSERT INTO `sys_logininfor` VALUES (438, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:08:39');
INSERT INTO `sys_logininfor` VALUES (439, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:08:58');
INSERT INTO `sys_logininfor` VALUES (440, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-28 23:09:05');
INSERT INTO `sys_logininfor` VALUES (441, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:09:50');
INSERT INTO `sys_logininfor` VALUES (442, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:11:06');
INSERT INTO `sys_logininfor` VALUES (443, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:14:56');
INSERT INTO `sys_logininfor` VALUES (444, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:29:46');
INSERT INTO `sys_logininfor` VALUES (445, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:51:00');
INSERT INTO `sys_logininfor` VALUES (446, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:52:06');
INSERT INTO `sys_logininfor` VALUES (447, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-28 23:58:30');
INSERT INTO `sys_logininfor` VALUES (448, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:01:19');
INSERT INTO `sys_logininfor` VALUES (449, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:02:26');
INSERT INTO `sys_logininfor` VALUES (450, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 00:02:29');
INSERT INTO `sys_logininfor` VALUES (451, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-29 00:25:59');
INSERT INTO `sys_logininfor` VALUES (452, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:34:03');
INSERT INTO `sys_logininfor` VALUES (453, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 00:34:08');
INSERT INTO `sys_logininfor` VALUES (454, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 00:34:14');
INSERT INTO `sys_logininfor` VALUES (455, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-29 00:34:21');
INSERT INTO `sys_logininfor` VALUES (456, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:34:27');
INSERT INTO `sys_logininfor` VALUES (457, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 00:34:42');
INSERT INTO `sys_logininfor` VALUES (458, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 00:35:44');
INSERT INTO `sys_logininfor` VALUES (459, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:37:27');
INSERT INTO `sys_logininfor` VALUES (460, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 00:37:38');
INSERT INTO `sys_logininfor` VALUES (461, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 00:44:20');
INSERT INTO `sys_logininfor` VALUES (462, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-29 00:44:53');
INSERT INTO `sys_logininfor` VALUES (463, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-29 01:01:46');
INSERT INTO `sys_logininfor` VALUES (464, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:01:51');
INSERT INTO `sys_logininfor` VALUES (465, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:01:53');
INSERT INTO `sys_logininfor` VALUES (466, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 01:03:14');
INSERT INTO `sys_logininfor` VALUES (467, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:07:40');
INSERT INTO `sys_logininfor` VALUES (468, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:07:51');
INSERT INTO `sys_logininfor` VALUES (469, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:19:15');
INSERT INTO `sys_logininfor` VALUES (470, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:19:24');
INSERT INTO `sys_logininfor` VALUES (471, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 01:24:04');
INSERT INTO `sys_logininfor` VALUES (472, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:24:16');
INSERT INTO `sys_logininfor` VALUES (473, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:24:19');
INSERT INTO `sys_logininfor` VALUES (474, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:28:12');
INSERT INTO `sys_logininfor` VALUES (475, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:28:40');
INSERT INTO `sys_logininfor` VALUES (476, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 01:30:09');
INSERT INTO `sys_logininfor` VALUES (477, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:31:25');
INSERT INTO `sys_logininfor` VALUES (478, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:31:46');
INSERT INTO `sys_logininfor` VALUES (479, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 01:33:26');
INSERT INTO `sys_logininfor` VALUES (480, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:33:33');
INSERT INTO `sys_logininfor` VALUES (481, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:34:37');
INSERT INTO `sys_logininfor` VALUES (482, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-29 01:34:39');
INSERT INTO `sys_logininfor` VALUES (483, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:36:14');
INSERT INTO `sys_logininfor` VALUES (484, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:37:59');
INSERT INTO `sys_logininfor` VALUES (485, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-29 01:39:08');
INSERT INTO `sys_logininfor` VALUES (486, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:39:12');
INSERT INTO `sys_logininfor` VALUES (487, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:39:42');
INSERT INTO `sys_logininfor` VALUES (488, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-29 01:40:52');
INSERT INTO `sys_logininfor` VALUES (489, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:40:56');
INSERT INTO `sys_logininfor` VALUES (490, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:41:24');
INSERT INTO `sys_logininfor` VALUES (491, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 01:53:15');
INSERT INTO `sys_logininfor` VALUES (492, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-29 23:18:17');
INSERT INTO `sys_logininfor` VALUES (493, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-30 12:40:12');
INSERT INTO `sys_logininfor` VALUES (494, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 12:40:16');
INSERT INTO `sys_logininfor` VALUES (495, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-30 12:40:28');
INSERT INTO `sys_logininfor` VALUES (496, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-05-30 12:40:31');
INSERT INTO `sys_logininfor` VALUES (497, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 12:40:35');
INSERT INTO `sys_logininfor` VALUES (498, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 19:55:01');
INSERT INTO `sys_logininfor` VALUES (499, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 20:21:39');
INSERT INTO `sys_logininfor` VALUES (500, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-30 20:36:18');
INSERT INTO `sys_logininfor` VALUES (501, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 20:36:26');
INSERT INTO `sys_logininfor` VALUES (502, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-05-30 20:36:33');
INSERT INTO `sys_logininfor` VALUES (503, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 20:43:08');
INSERT INTO `sys_logininfor` VALUES (504, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-30 21:43:05');
INSERT INTO `sys_logininfor` VALUES (505, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 19:38:04');
INSERT INTO `sys_logininfor` VALUES (506, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:14:05');
INSERT INTO `sys_logininfor` VALUES (507, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:16:10');
INSERT INTO `sys_logininfor` VALUES (508, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:16:46');
INSERT INTO `sys_logininfor` VALUES (509, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:17:05');
INSERT INTO `sys_logininfor` VALUES (510, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:21:56');
INSERT INTO `sys_logininfor` VALUES (511, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:27:49');
INSERT INTO `sys_logininfor` VALUES (512, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-31 20:30:52');
INSERT INTO `sys_logininfor` VALUES (513, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:30:54');
INSERT INTO `sys_logininfor` VALUES (514, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:31:27');
INSERT INTO `sys_logininfor` VALUES (515, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:31:36');
INSERT INTO `sys_logininfor` VALUES (516, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:35:58');
INSERT INTO `sys_logininfor` VALUES (517, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:37:50');
INSERT INTO `sys_logininfor` VALUES (518, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:38:17');
INSERT INTO `sys_logininfor` VALUES (519, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:39:46');
INSERT INTO `sys_logininfor` VALUES (520, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:40:38');
INSERT INTO `sys_logininfor` VALUES (521, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:42:13');
INSERT INTO `sys_logininfor` VALUES (522, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:43:32');
INSERT INTO `sys_logininfor` VALUES (523, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:43:55');
INSERT INTO `sys_logininfor` VALUES (524, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:44:10');
INSERT INTO `sys_logininfor` VALUES (525, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:48:27');
INSERT INTO `sys_logininfor` VALUES (526, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 20:51:55');
INSERT INTO `sys_logininfor` VALUES (527, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:11:41');
INSERT INTO `sys_logininfor` VALUES (528, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:11:57');
INSERT INTO `sys_logininfor` VALUES (529, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:12:06');
INSERT INTO `sys_logininfor` VALUES (530, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:16:03');
INSERT INTO `sys_logininfor` VALUES (531, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:16:09');
INSERT INTO `sys_logininfor` VALUES (532, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:17:22');
INSERT INTO `sys_logininfor` VALUES (533, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:18:40');
INSERT INTO `sys_logininfor` VALUES (534, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-31 21:18:56');
INSERT INTO `sys_logininfor` VALUES (535, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:18:59');
INSERT INTO `sys_logininfor` VALUES (536, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 21:23:58');
INSERT INTO `sys_logininfor` VALUES (537, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 23:53:24');
INSERT INTO `sys_logininfor` VALUES (538, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-31 23:53:45');
INSERT INTO `sys_logininfor` VALUES (539, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 23:53:51');
INSERT INTO `sys_logininfor` VALUES (540, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-05-31 23:54:38');
INSERT INTO `sys_logininfor` VALUES (541, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-05-31 23:57:24');
INSERT INTO `sys_logininfor` VALUES (542, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:02:22');
INSERT INTO `sys_logininfor` VALUES (543, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:04:33');
INSERT INTO `sys_logininfor` VALUES (544, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:05:29');
INSERT INTO `sys_logininfor` VALUES (545, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:07:33');
INSERT INTO `sys_logininfor` VALUES (546, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:08:11');
INSERT INTO `sys_logininfor` VALUES (547, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 00:09:14');
INSERT INTO `sys_logininfor` VALUES (548, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 19:44:55');
INSERT INTO `sys_logininfor` VALUES (549, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 19:47:49');
INSERT INTO `sys_logininfor` VALUES (550, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 19:56:10');
INSERT INTO `sys_logininfor` VALUES (551, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 19:57:52');
INSERT INTO `sys_logininfor` VALUES (552, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 19:57:56');
INSERT INTO `sys_logininfor` VALUES (553, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:00:06');
INSERT INTO `sys_logininfor` VALUES (554, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:05:15');
INSERT INTO `sys_logininfor` VALUES (555, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:06:33');
INSERT INTO `sys_logininfor` VALUES (556, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 20:13:56');
INSERT INTO `sys_logininfor` VALUES (557, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:13:59');
INSERT INTO `sys_logininfor` VALUES (558, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 20:28:39');
INSERT INTO `sys_logininfor` VALUES (559, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:28:43');
INSERT INTO `sys_logininfor` VALUES (560, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 20:34:42');
INSERT INTO `sys_logininfor` VALUES (561, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:34:45');
INSERT INTO `sys_logininfor` VALUES (562, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:36:28');
INSERT INTO `sys_logininfor` VALUES (563, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:40:47');
INSERT INTO `sys_logininfor` VALUES (564, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:59:41');
INSERT INTO `sys_logininfor` VALUES (565, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 20:59:47');
INSERT INTO `sys_logininfor` VALUES (566, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:00:12');
INSERT INTO `sys_logininfor` VALUES (567, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:01:15');
INSERT INTO `sys_logininfor` VALUES (568, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 21:01:24');
INSERT INTO `sys_logininfor` VALUES (569, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:02:09');
INSERT INTO `sys_logininfor` VALUES (570, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-06-01 21:02:11');
INSERT INTO `sys_logininfor` VALUES (571, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:03:29');
INSERT INTO `sys_logininfor` VALUES (572, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-06-01 21:03:29');
INSERT INTO `sys_logininfor` VALUES (573, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-01 21:03:30');
INSERT INTO `sys_logininfor` VALUES (574, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:04:32');
INSERT INTO `sys_logininfor` VALUES (575, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:10:03');
INSERT INTO `sys_logininfor` VALUES (576, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-01 21:13:39');
INSERT INTO `sys_logininfor` VALUES (577, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:19:00');
INSERT INTO `sys_logininfor` VALUES (578, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:22:03');
INSERT INTO `sys_logininfor` VALUES (579, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:23:31');
INSERT INTO `sys_logininfor` VALUES (580, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:46:04');
INSERT INTO `sys_logininfor` VALUES (581, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:48:40');
INSERT INTO `sys_logininfor` VALUES (582, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:51:03');
INSERT INTO `sys_logininfor` VALUES (583, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 21:55:02');
INSERT INTO `sys_logininfor` VALUES (584, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-01 22:00:31');
INSERT INTO `sys_logininfor` VALUES (585, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 09:47:05');
INSERT INTO `sys_logininfor` VALUES (586, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 10:32:51');
INSERT INTO `sys_logininfor` VALUES (587, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 10:36:28');
INSERT INTO `sys_logininfor` VALUES (588, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 12:03:20');
INSERT INTO `sys_logininfor` VALUES (589, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 13:01:28');
INSERT INTO `sys_logininfor` VALUES (590, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 16:15:14');
INSERT INTO `sys_logininfor` VALUES (591, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 16:55:08');
INSERT INTO `sys_logininfor` VALUES (592, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-05 18:14:30');
INSERT INTO `sys_logininfor` VALUES (593, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 18:14:34');
INSERT INTO `sys_logininfor` VALUES (594, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-05 19:31:35');
INSERT INTO `sys_logininfor` VALUES (595, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 08:19:52');
INSERT INTO `sys_logininfor` VALUES (596, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 08:27:25');
INSERT INTO `sys_logininfor` VALUES (597, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 09:00:28');
INSERT INTO `sys_logininfor` VALUES (598, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 09:30:47');
INSERT INTO `sys_logininfor` VALUES (599, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 10:05:39');
INSERT INTO `sys_logininfor` VALUES (600, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 10:22:17');
INSERT INTO `sys_logininfor` VALUES (601, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 10:37:14');
INSERT INTO `sys_logininfor` VALUES (602, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 10:37:35');
INSERT INTO `sys_logininfor` VALUES (603, 'ry', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 10:37:50');
INSERT INTO `sys_logininfor` VALUES (604, 'ry', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 10:42:13');
INSERT INTO `sys_logininfor` VALUES (605, 'ry', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 10:42:20');
INSERT INTO `sys_logininfor` VALUES (606, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-06 11:13:18');
INSERT INTO `sys_logininfor` VALUES (607, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 11:13:21');
INSERT INTO `sys_logininfor` VALUES (608, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-06 12:08:35');
INSERT INTO `sys_logininfor` VALUES (609, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 12:08:43');
INSERT INTO `sys_logininfor` VALUES (610, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 12:18:55');
INSERT INTO `sys_logininfor` VALUES (611, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 12:19:01');
INSERT INTO `sys_logininfor` VALUES (612, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-06 12:19:08');
INSERT INTO `sys_logininfor` VALUES (613, 'ruoyi', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '用户不存在/密码错误', '2021-06-06 12:19:15');
INSERT INTO `sys_logininfor` VALUES (614, 'ry', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-06 12:19:25');
INSERT INTO `sys_logininfor` VALUES (615, 'ry', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 12:19:30');
INSERT INTO `sys_logininfor` VALUES (616, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 20:20:36');
INSERT INTO `sys_logininfor` VALUES (617, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-06 22:02:49');
INSERT INTO `sys_logininfor` VALUES (618, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 19:36:26');
INSERT INTO `sys_logininfor` VALUES (619, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 20:32:16');
INSERT INTO `sys_logininfor` VALUES (620, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-08 20:52:21');
INSERT INTO `sys_logininfor` VALUES (621, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-08 20:52:24');
INSERT INTO `sys_logininfor` VALUES (622, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 20:52:27');
INSERT INTO `sys_logininfor` VALUES (623, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 21:02:59');
INSERT INTO `sys_logininfor` VALUES (624, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 21:34:49');
INSERT INTO `sys_logininfor` VALUES (625, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-08 21:54:07');
INSERT INTO `sys_logininfor` VALUES (626, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 21:54:11');
INSERT INTO `sys_logininfor` VALUES (627, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-08 22:57:18');
INSERT INTO `sys_logininfor` VALUES (628, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-06-09 19:57:22');
INSERT INTO `sys_logininfor` VALUES (629, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-06-09 20:09:19');
INSERT INTO `sys_logininfor` VALUES (630, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-09 20:09:23');
INSERT INTO `sys_logininfor` VALUES (631, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-10 19:33:48');
INSERT INTO `sys_logininfor` VALUES (632, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-10 20:06:30');
INSERT INTO `sys_logininfor` VALUES (633, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-10 21:46:59');
INSERT INTO `sys_logininfor` VALUES (634, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-06-12 14:45:40');
INSERT INTO `sys_logininfor` VALUES (635, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-12 14:50:57');
INSERT INTO `sys_logininfor` VALUES (636, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-12 19:07:41');
INSERT INTO `sys_logininfor` VALUES (637, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-06-12 20:19:49');
INSERT INTO `sys_logininfor` VALUES (638, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-12 21:51:14');
INSERT INTO `sys_logininfor` VALUES (639, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-12 23:54:27');
INSERT INTO `sys_logininfor` VALUES (640, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-13 13:58:17');
INSERT INTO `sys_logininfor` VALUES (641, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-13 13:58:21');
INSERT INTO `sys_logininfor` VALUES (642, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-13 20:01:39');
INSERT INTO `sys_logininfor` VALUES (643, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-13 21:06:09');
INSERT INTO `sys_logininfor` VALUES (644, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-13 21:22:12');
INSERT INTO `sys_logininfor` VALUES (645, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 09:48:48');
INSERT INTO `sys_logininfor` VALUES (646, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 10:31:01');
INSERT INTO `sys_logininfor` VALUES (647, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 10:51:01');
INSERT INTO `sys_logininfor` VALUES (648, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-14 10:53:48');
INSERT INTO `sys_logininfor` VALUES (649, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 10:53:51');
INSERT INTO `sys_logininfor` VALUES (650, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 10:56:26');
INSERT INTO `sys_logininfor` VALUES (651, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 10:57:38');
INSERT INTO `sys_logininfor` VALUES (652, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 11:01:34');
INSERT INTO `sys_logininfor` VALUES (653, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 11:07:35');
INSERT INTO `sys_logininfor` VALUES (654, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-06-14 11:19:49');
INSERT INTO `sys_logininfor` VALUES (655, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 11:20:20');
INSERT INTO `sys_logininfor` VALUES (656, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 12:07:39');
INSERT INTO `sys_logininfor` VALUES (657, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-14 13:30:42');
INSERT INTO `sys_logininfor` VALUES (658, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 13:30:46');
INSERT INTO `sys_logininfor` VALUES (659, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-06-14 14:08:29');
INSERT INTO `sys_logininfor` VALUES (660, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 14:08:38');
INSERT INTO `sys_logininfor` VALUES (661, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 14:39:20');
INSERT INTO `sys_logininfor` VALUES (662, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 15:18:54');
INSERT INTO `sys_logininfor` VALUES (663, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-06-14 17:44:03');
INSERT INTO `sys_logininfor` VALUES (664, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 17:44:20');
INSERT INTO `sys_logininfor` VALUES (665, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 18:14:37');
INSERT INTO `sys_logininfor` VALUES (666, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-14 18:38:42');
INSERT INTO `sys_logininfor` VALUES (667, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-16 00:45:07');
INSERT INTO `sys_logininfor` VALUES (668, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-06-16 22:33:00');
INSERT INTO `sys_logininfor` VALUES (669, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-16 22:35:17');
INSERT INTO `sys_logininfor` VALUES (670, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-20 08:02:10');
INSERT INTO `sys_logininfor` VALUES (671, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-06-20 10:20:16');
INSERT INTO `sys_logininfor` VALUES (672, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-06-20 10:20:19');
INSERT INTO `sys_logininfor` VALUES (673, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-02 20:18:28');
INSERT INTO `sys_logininfor` VALUES (674, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-03 09:02:41');
INSERT INTO `sys_logininfor` VALUES (675, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-03 10:35:57');
INSERT INTO `sys_logininfor` VALUES (676, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-03 11:10:28');
INSERT INTO `sys_logininfor` VALUES (677, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-03 14:46:51');
INSERT INTO `sys_logininfor` VALUES (678, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-03 16:11:53');
INSERT INTO `sys_logininfor` VALUES (679, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-05 19:35:10');
INSERT INTO `sys_logininfor` VALUES (680, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-05 20:35:38');
INSERT INTO `sys_logininfor` VALUES (681, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-05 21:22:47');
INSERT INTO `sys_logininfor` VALUES (682, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '1', '验证码错误', '2021-07-06 20:50:44');
INSERT INTO `sys_logininfor` VALUES (683, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-07-06 20:50:47');
INSERT INTO `sys_logininfor` VALUES (684, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-06 21:46:14');
INSERT INTO `sys_logininfor` VALUES (685, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-07 19:23:05');
INSERT INTO `sys_logininfor` VALUES (686, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-10 10:28:18');
INSERT INTO `sys_logininfor` VALUES (687, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-10 10:59:38');
INSERT INTO `sys_logininfor` VALUES (688, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-10 13:35:44');
INSERT INTO `sys_logininfor` VALUES (689, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-07-17 19:42:40');
INSERT INTO `sys_logininfor` VALUES (690, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-09 21:53:23');
INSERT INTO `sys_logininfor` VALUES (691, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 19:01:32');
INSERT INTO `sys_logininfor` VALUES (692, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 19:01:38');
INSERT INTO `sys_logininfor` VALUES (693, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-14 19:07:23');
INSERT INTO `sys_logininfor` VALUES (694, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 19:07:57');
INSERT INTO `sys_logininfor` VALUES (695, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-14 19:08:31');
INSERT INTO `sys_logininfor` VALUES (696, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 19:08:39');
INSERT INTO `sys_logininfor` VALUES (697, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-14 19:13:08');
INSERT INTO `sys_logininfor` VALUES (698, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 19:20:19');
INSERT INTO `sys_logininfor` VALUES (699, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-14 20:20:49');
INSERT INTO `sys_logininfor` VALUES (700, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-15 16:49:22');
INSERT INTO `sys_logininfor` VALUES (701, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-15 16:54:31');
INSERT INTO `sys_logininfor` VALUES (702, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-15 18:02:40');
INSERT INTO `sys_logininfor` VALUES (703, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-16 18:26:23');
INSERT INTO `sys_logininfor` VALUES (704, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-16 19:11:33');
INSERT INTO `sys_logininfor` VALUES (705, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-18 18:36:30');
INSERT INTO `sys_logininfor` VALUES (706, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-18 18:41:51');
INSERT INTO `sys_logininfor` VALUES (707, 'thh', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '注册成功', '2021-08-18 19:48:56');
INSERT INTO `sys_logininfor` VALUES (708, 'thh', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-18 19:49:30');
INSERT INTO `sys_logininfor` VALUES (709, 'thh', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-18 20:11:05');
INSERT INTO `sys_logininfor` VALUES (710, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-18 20:11:10');
INSERT INTO `sys_logininfor` VALUES (711, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-19 18:58:32');
INSERT INTO `sys_logininfor` VALUES (712, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-21 10:14:45');
INSERT INTO `sys_logininfor` VALUES (713, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码错误', '2021-08-21 11:59:52');
INSERT INTO `sys_logininfor` VALUES (714, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-21 11:59:55');
INSERT INTO `sys_logininfor` VALUES (715, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-21 12:29:31');
INSERT INTO `sys_logininfor` VALUES (716, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-21 14:05:43');
INSERT INTO `sys_logininfor` VALUES (717, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-21 16:04:29');
INSERT INTO `sys_logininfor` VALUES (718, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 00:31:37');
INSERT INTO `sys_logininfor` VALUES (719, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-22 00:33:05');
INSERT INTO `sys_logininfor` VALUES (720, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 00:34:00');
INSERT INTO `sys_logininfor` VALUES (721, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-22 00:34:08');
INSERT INTO `sys_logininfor` VALUES (722, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 01:12:48');
INSERT INTO `sys_logininfor` VALUES (723, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 09:14:06');
INSERT INTO `sys_logininfor` VALUES (724, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 09:44:47');
INSERT INTO `sys_logininfor` VALUES (725, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 10:29:47');
INSERT INTO `sys_logininfor` VALUES (726, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 12:22:38');
INSERT INTO `sys_logininfor` VALUES (727, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-22 15:13:45');
INSERT INTO `sys_logininfor` VALUES (728, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 17:06:06');
INSERT INTO `sys_logininfor` VALUES (729, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 17:24:32');
INSERT INTO `sys_logininfor` VALUES (730, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-22 22:45:58');
INSERT INTO `sys_logininfor` VALUES (731, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-22 22:50:31');
INSERT INTO `sys_logininfor` VALUES (732, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-23 20:01:54');
INSERT INTO `sys_logininfor` VALUES (733, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-23 21:53:50');
INSERT INTO `sys_logininfor` VALUES (734, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '1', '验证码已失效', '2021-08-24 19:12:20');
INSERT INTO `sys_logininfor` VALUES (735, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-24 19:12:20');
INSERT INTO `sys_logininfor` VALUES (736, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-25 18:41:42');
INSERT INTO `sys_logininfor` VALUES (737, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-25 20:59:59');
INSERT INTO `sys_logininfor` VALUES (738, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-26 20:50:09');
INSERT INTO `sys_logininfor` VALUES (739, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-27 19:31:04');
INSERT INTO `sys_logininfor` VALUES (740, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-27 19:55:16');
INSERT INTO `sys_logininfor` VALUES (741, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 09:54:24');
INSERT INTO `sys_logininfor` VALUES (742, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 10:31:25');
INSERT INTO `sys_logininfor` VALUES (743, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 12:22:40');
INSERT INTO `sys_logininfor` VALUES (744, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 12:58:51');
INSERT INTO `sys_logininfor` VALUES (745, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-28 13:40:30');
INSERT INTO `sys_logininfor` VALUES (746, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 15:28:19');
INSERT INTO `sys_logininfor` VALUES (747, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '退出成功', '2021-08-28 17:15:52');
INSERT INTO `sys_logininfor` VALUES (748, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 17:15:56');
INSERT INTO `sys_logininfor` VALUES (749, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-28 19:17:56');
INSERT INTO `sys_logininfor` VALUES (750, 'admin', '127.0.0.1', '内网IP', 'Chrome 9', 'Windows 10', '0', '登录成功', '2021-08-29 09:09:50');
INSERT INTO `sys_logininfor` VALUES (751, 'admin', '127.0.0.1', '内网IP', 'Chrome 8', 'Windows 10', '0', '登录成功', '2021-08-29 09:30:56');
...
INSERT INTO `sys_menu` VALUES (4, '若依官网', 0, 4, 'http://ruoyi.vip', NULL, 0, 0, 'M', '0', '0', '', 'guide', 'admin', '2021-08-14 17:38:09', '', NULL, '若依官网地址');
...
INSERT INTO `sys_menu` VALUES (2043, '文章收藏', 2036, 5, 'collect', 'blog/collect/index', 1, 0, 'C', '0', '0', 'blog:collect:list', 'cascader', 'admin', '2021-08-28 13:36:33', 'admin', '2021-08-28 13:58:42', '文章收藏菜单');
INSERT INTO `sys_menu` VALUES (2044, '文章收藏查询', 2043, 1, '#', '', 1, 0, 'F', '0', '0', 'blog:collect:query', '#', 'admin', '2021-08-28 13:36:33', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2045, '文章收藏新增', 2043, 2, '#', '', 1, 0, 'F', '0', '0', 'blog:collect:add', '#', 'admin', '2021-08-28 13:36:33', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2046, '文章收藏修改', 2043, 3, '#', '', 1, 0, 'F', '0', '0', 'blog:collect:edit', '#', 'admin', '2021-08-28 13:36:33', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2047, '文章收藏删除', 2043, 4, '#', '', 1, 0, 'F', '0', '0', 'blog:collect:remove', '#', 'admin', '2021-08-28 13:36:33', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2048, '文章收藏导出', 2043, 5, '#', '', 1, 0, 'F', '0', '0', 'blog:collect:export', '#', 'admin', '2021-08-28 13:36:33', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2049, '博客分类', 2036, 3, 'sort', 'blog/sort/index', 1, 0, 'C', '0', '0', 'blog:sort:list', 'icon', 'admin', '2021-08-28 13:36:44', 'admin', '2021-08-28 13:58:03', '博客分类菜单');
INSERT INTO `sys_menu` VALUES (2050, '博客分类查询', 2049, 1, '#', '', 1, 0, 'F', '0', '0', 'blog:sort:query', '#', 'admin', '2021-08-28 13:36:44', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2051, '博客分类新增', 2049, 2, '#', '', 1, 0, 'F', '0', '0', 'blog:sort:add', '#', 'admin', '2021-08-28 13:36:44', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2052, '博客分类修改', 2049, 3, '#', '', 1, 0, 'F', '0', '0', 'blog:sort:edit', '#', 'admin', '2021-08-28 13:36:44', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2053, '博客分类删除', 2049, 4, '#', '', 1, 0, 'F', '0', '0', 'blog:sort:remove', '#', 'admin', '2021-08-28 13:36:44', '', NULL, '');
INSERT INTO `sys_menu` VALUES (2054, '博客分类导出', 2049, 5, '#', '', 1, 0, 'F', '0', '0', 'blog:sort:export', '#', 'admin', '2021-08-28 13:36:44', '', NULL, '');
...
) ENGINE = InnoDB AUTO_INCREMENT = 2394 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '操作日志记录' ROW_FORMAT = Dynamic;
...
-- Records of sys_user
-- ----------------------------
INSERT INTO `sys_user` VALUES (1, 103, 'admin', '若依', '00', 'ry@163.com', '15888888888', '1', '/profile/avatar/2021/08/16/064230e0-e311-44da-9365-f09a8d1f2435.jpeg', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', '2021-08-29 09:30:57', 'admin', '2021-04-29 21:28:12', '', '2021-08-29 09:30:56', '管理员');
INSERT INTO `sys_user` VALUES (2, 105, 'ry', '若依', '00', 'ry@qq.com', '15666666666', '1', '/profile/avatar/defaul/avatar.jpg', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '0', '0', '127.0.0.1', '2021-04-29 21:28:12', 'admin', '2021-04-29 21:28:12', '', NULL, '测试员');
...
-- ----------------------------
INSERT INTO `website_banner` VALUES (1, '测试', '/movie/images/2021/05/03/f6dd8c57-cff8-475c-89c0-8310d7d929fa.jpg', 'http://www.ithhit.cn', 1, '0', '2021-05-05 16:33:06', '2021-08-16 19:35:27', NULL, NULL, '1');
INSERT INTO `website_banner` VALUES (2, '测试2', '/upload/2021/08/16/5a44fccb-9f34-4604-a2d1-529fb71e05b4.jpg', 'http://baidu.com', 0, '0', '2021-05-05 17:49:21', '2021-08-16 19:32:20', NULL, NULL, '1');
...
CREATE TABLE `website_collect`  (
  ...
  `sid` varchar(32) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '页面id',
...
CREATE TABLE `website_comment`  (
  ...
  `sid` bigint(20) NULL DEFAULT NULL COMMENT '被评论的文章电影或者页面的ID',
...
) ENGINE = InnoDB AUTO_INCREMENT = 161 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '评论表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of website_comment
-- ----------------------------
INSERT INTO `website_comment` VALUES (156, 94, 1, NULL, NULL, '若依', '/profile/avatar/2021/06/05/4cd38078-558f-4f78-ae01-7e9cca4f07e3.jpeg', 'ry@163.com', '/movie/94', 'APPROVED', '127.0.0.1', NULL, NULL, '内网IP', 'Windows 10', NULL, 'Chrome 9 91.0.4472.77', NULL, '<span class=\"emoji-item-common emoji-relaxed emoji-size-small\"></span>', NULL, 0, 0, '2021-06-14 15:34:49', '2021-06-14 15:34:49', 'wm_movie');
INSERT INTO `website_comment` VALUES (157, 95, 1, NULL, NULL, '若依', '/profile/avatar/2021/06/05/4cd38078-558f-4f78-ae01-7e9cca4f07e3.jpeg', 'ry@163.com', '/movie/95', 'APPROVED', '127.0.0.1', NULL, NULL, '内网IP', 'Windows 10', NULL, 'Chrome 9 91.0.4472.77', NULL, '1212', NULL, 0, 0, '2021-07-03 16:11:58', '2021-07-03 16:11:58', 'wm_movie');
INSERT INTO `website_comment` VALUES (160, 128, 1, NULL, NULL, '若依', '/profile/avatar/2021/08/16/064230e0-e311-44da-9365-f09a8d1f2435.jpeg', 'ry@163.com', '/movie/128', 'APPROVED', '127.0.0.1', NULL, NULL, '内网IP', 'Windows 10', NULL, 'Chrome 9 91.0.4472.77', NULL, '<span class=\"emoji-item-common emoji-innocent emoji-size-small\"></span>\n<span class=\"emoji-item-common emoji-heart_eyes emoji-size-small\"></span>\n<span class=\"emoji-item-common emoji-wink emoji-size-small\"></span>\n<span class=\"emoji-item-common emoji-unamused emoji-size-small\"></span>', NULL, 0, 0, '2021-08-25 21:01:17', '2021-08-25 21:01:17', 'wm_movie');
...
) ENGINE = InnoDB AUTO_INCREMENT = 8 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '站点友情链接' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of website_link
-- ----------------------------
INSERT INTO `website_link` VALUES (1, '2019-06-20 15:57:56', '2021-08-24 19:26:55', 1, 90, '蘑菇博客', 'http://www.ithhit.cn', '_blank', NULL, NULL, NULL);
INSERT INTO `website_link` VALUES (2, '2019-06-26 18:17:18', '2021-08-24 19:26:59', 1, 100, '百度', 'http://www.baidu.com', '_blank', NULL, NULL, NULL);
...
-- ----------------------------
INSERT INTO `website_web_config` VALUES ('web_config', '/common/image/2021/08/24/6bc541e4538f4e019b6e82285433c06f/6bc541e4538f4e019b6e82285433c06f.jpg', '辉皇影院', '一个专注于技术分享的博客平台，大家以共同学习，乐于分享，拥抱开源的价值观进行学习交流', '辉皇影院,辉皇社区,开发者社区', '陌溪', '赣ICP备18014504号', 1, '2021-06-08 22:25:55', '2021-08-28 17:31:50', '一个专注于技术分享的博客平台', 'b4bb7ba9905203188eb89895f0e7c121', '0a55fb8d7181493e4fe781a08ff522ad', 'https://github.com/moxi624', 'https://gitee.com/moxi159753', '770666529', '950309755', '1111111', '[\"2\",\"1\"]', '[\"4\",\"2\",\"1\",\"5\",\"3\"]', '1', 0, 1, 0, '22', '3', '4', 'na0', '0', '111', 'image.ithhit.cn', '770666529@qq.com', '22', '333', '222', '111', NULL, NULL, '1', '1', '1', '1', 1, NULL, NULL, '1', 'admin', NULL, NULL, '1');
...
CREATE TABLE `wm_movie`  (
  ...
  `type` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '电影类型',
  ...
  `likes_count` int(11) NULL DEFAULT NULL COMMENT '喜欢数',
  `unlikes_count` int(11) NULL DEFAULT NULL COMMENT '不喜欢数',
  ...
  `is_download` char(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '是否可以下载',
...
) ENGINE = InnoDB AUTO_INCREMENT = 135 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '电影表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of wm_movie
-- ----------------------------
INSERT INTO `wm_movie` VALUES (128, '/showFile/movie/image/天龙八部.png', '天龙八部', 'teleplay', 'china', '大宋,北宋,天龙八部,KungFu,年间,国土', '<p>天龙八部是一部动作片类型的电影，于1997在香港上映，目前的语言版本有汉语普通话。大理国王子段誉（汤镇业 饰）生性善良，喜文厌武，为逃避父皇严加管教，段誉时常杳无踪影，却在一山洞身受，天龙八部播放地址获取自网路，如有侵权请联系...</p>', 'admin', '2021-08-01 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '稻田影视每日更新大量最新电影电视剧，为喜爱影视的朋友提供在线观影的平台 动作片天龙八部是萧笙的一部作品，由徐少强,汤镇业,黄日华,陈玉莲参演，当前被点击了1438次，平均评分4.9。大家有什么好的影视作品欢迎留言投稿！\n大理国王子段誉（汤镇业 饰）生性善良，喜文厌武，为逃避父皇严加管教，段誉时常杳无踪影，却在一山洞身受巨毒，平添功力无限，还在一仙女塑像前得武功密笈一本　　为完成复国大计，慕容复（惠天赐 饰）已掌握除《降龙十八掌》、《六脉神剑》之外的所有武功绝学在曼陀山庄见到慕容公子的表妹王语嫣（陈玉莲 饰）时，段誉以为王姑娘就是自己在山洞见到的那个仙女，…大理国王子段誉（汤镇业 饰）生性善良，喜文厌武，为逃避父皇严加管教，段誉时常杳无踪影，却在一山洞身受巨毒，平添功力无限，还在一仙女塑像前得武功密笈一本　　为完成复国大计，慕容复（惠天赐 饰）已掌握除《降龙十八掌》、《六脉神剑》之外的所有武功绝学。在曼陀山庄见到慕容公子的表妹王语嫣（陈玉莲 饰）时，段誉以为王姑娘就是自己在山洞见到的那个仙女，顿生爱慕之情。另一边，段誉偶遇天下仁义名侠丐帮帮主乔峰（徐少强 饰）。后来乔峰才知自己是契丹人，回到契丹的乔峰因阻止国王进兵大宋身陷囹圄。为救乔峰，段誉、虚竹（黄日华 饰）前往契丹，救乔峰至契丹与大宋的边界雁门关。乔峰没有与中原继续为敌，而是为阻止契丹与大宋交兵殃及百姓，自行了断性命，为契丹和中原的安宁留下一段佳话。', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 4.6);
INSERT INTO `wm_movie` VALUES (129, '/showFile/movie/image/除暴.png', '除暴', ' film', 'HongKong', '刑警, 追捕,集团,悍匪', '<p>除暴是一部动作片类型的电影，于2020在香港上映，目前的语言版本有国语。上世纪90年代，刑警钟诚受命追捕悍匪集团老鹰帮活下去的诀窍是：保持愚蠢，又不能知道自己有多蠢。 --，除暴播放地址获取自网路，如有侵权请联系...</p>', 'admin', '2020-08-01 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '辉皇影视每日更新大量最新电影电视剧，为喜爱影视的朋友提供在线观影的平台 动作片除暴是刘浩良的一部作品，由王千源,吴彦祖,春夏,卫诗雅,大力,于笑,胡子程,李晓川参演，当前被点击了15851次，平均评分3.0。大家有什么好的影视作品欢迎留言投稿！\n上世纪90年代，刑警钟诚受命追捕悍匪集团老鹰帮活下去的诀窍是：保持愚蠢，又不能知道自己有多蠢。 ----王小波这群悍匪犯下惊天连环劫案，训练有素且纪律严明，首领张隼更屡次恶意挑衅，矛头直指钟诚。为将老鹰帮捉拿归案，钟诚带领刑警小队咬死不放，誓与恶势力斗争到底。数年间，警匪上演了一次次紧张刺激的较量，悍匪愈加猖獗，警方步步逼近，双方展开殊死对决……\n', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 3.0);
INSERT INTO `wm_movie` VALUES (131, '/showFile/movie/image/你是我的荣耀.png', '你是我的荣耀', 'teleplay', 'china', '网游,游戏,荣耀', '<p>你是我的荣耀是一部国产剧类型的电视剧，于2021在中国大陆上映，目前的语言版本有国语。《你是我的荣耀》讲述了人气女星“乔晶晶”与高中时单恋的学神“于途”，在阔别十年后，于线上再度重逢，开，你是我的荣耀播放地址获取自网路，如有侵权请联系...</p><p><br></p>', 'admin', '2021-08-24 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '你是我的荣耀是王之的一部作品，由迪丽热巴,杨洋,潘粤明,胡可,王彦霖,郑合惠子,金晨,吴倩,高露,涂松岩,季肖冰,孙雅丽参演，当前被点击了1251次，平均评分2.0。大家有什么好的影视作品欢迎留言投稿！\n《你是我的荣耀》讲述了人气女星“乔晶晶”与高中时单恋的学神“于途”，在阔别十年后，于线上再度重逢，开启了一段浪漫治愈的暖爱之实际上这个世界上没有好人和坏人，只有做了好事的人，和做了坏事的人。 ----柴静。时光匆匆，此时“于途”已成为心怀梦想的航天设计师，“乔晶晶”也成为星光闪耀的当红女演员，二人在追梦路上努力坚守，在浪漫奇遇中相互鼓励，终成彼此荣耀！\n', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 2.0);
INSERT INTO `wm_movie` VALUES (132, '/showFile/movie/image/天下无贼.png', '天下无贼', ' film', 'china', 'KungFu, 天下无贼,城市', '<p>天下无贼是一部爱情片类型的电影，于2004在大陆上映，目前的语言版本有。男贼王薄和女贼王丽是一对扒窃搭档，也是一对浪迹天涯的亡命恋人他们在一列火车上遇到了一个名叫傻根的农民，天下无贼播放地址获取自网路，如有侵权请联系...</p>', 'admin', '2021-08-25 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '天下无贼是冯小刚的一部作品，由刘若英,王宝强,刘德华参演，当前被点击了1020次，平均评分4.0。大家有什么好的影视作品欢迎留言投稿！\n男贼王薄和女贼王丽是一对扒窃搭档，也是一对浪迹天涯的亡命恋人他们在一列火车上遇到了一个名叫傻根的农民，他刚刚从城市里挣了一笔钱要回老家盖房子娶媳妇。傻根不相信天下有贼，王薄和王丽最初想对他下手，后来却被他的纯朴所打动，决定保护傻根，圆他一个天下无贼的梦想，并由此与另一个扒窃团伙引发了一系列的明争暗斗。该团伙头目黎叔意欲收服王薄遭拒，该团伙其他成员与王薄比试皆败下阵来...', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 4.7);
INSERT INTO `wm_movie` VALUES (133, '/showFile/movie/image/寒战.png', '寒战', ' film', 'HongKong', '金融中心,艰难险阻,城市,刑警', '<p>寒战是一部动作片类型的电影，于2012在大陆上映，目前的语言版本有。作为亚洲金融中心的香港被称作“最安全的城市”，岂料警局接到一通匿名电话，对方声称劫持了警队的一辆前线，寒战播放地址获取自网路，如有侵权请联系...</p><p>作为亚洲金融中心的香港被称作“最安全的城市”，岂料警局接到一通匿名电话，对方声称劫持了警队的一辆前线冲锋车以及车内的五名警员和武器装备此次事件引起了香港警队高层的高度重视，适逢处长出访国外，此时由鹰派人物行动副处长李文彬（梁家辉 饰）与年轻的管理副处长刘杰辉（郭富城 饰）一起负责这次案件，并将行动命名为“寒战”，李文彬任总指挥。李文彬的儿子李家俊（彭于晏 饰）也是人质之一，刘杰辉因此对李文彬父子产生怀疑。与此同时，匪徒展开了进一步行动：警察被杀、爆炸案频发……到底是谁在将整个警队玩弄于鼓掌之中？香港真的是“最安全的城市”吗？</p><p><br></p><p><br></p>', 'admin', '2012-08-01 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '寒战是一部动作片类型的电影，于2012在大陆上映，目前的语言版本有。作为亚洲金融中心的香港被称作“最安全的城市”，岂料警局接到一通匿名电话，对方声称劫持了警队的一辆前线，寒战播放地址获取自网路，如有侵权请联系...\n\n作为亚洲金融中心的香港被称作“最安全的城市”，岂料警局接到一通匿名电话，对方声称劫持了警队的一辆前线冲锋车以及车内的五名警员和武器装备此次事件引起了香港警队高层的高度重视，适逢处长出访国外，此时由鹰派人物行动副处长李文彬（梁家辉 饰）与年轻的管理副处长刘杰辉（郭富城 饰）一起负责这次案件，并将行动命名为“寒战”，李文彬任总指挥。李文彬的儿子李家俊（彭于晏 饰）也是人质之一，刘杰辉因此对李文彬父子产生怀疑。与此同时，匪徒展开了进一步行动：警察被杀、爆炸案频发……到底是谁在将整个警队玩弄于鼓掌之中？香港真的是“最安全的城市”吗？\n', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 4.9);
INSERT INTO `wm_movie` VALUES (134, '/showFile/movie/image/白蛇.png', '白蛇', 'cartoon', 'china', '动漫版', '<p>白蛇：缘起 动漫版是一部动漫类型的，于2019在内地上映，目前的语言版本有普通话。失忆少女小白被捕蛇人阿宣所救，为了解开自己的身份谜团，小白在阿宣的帮助下开始寻找线当你真正爱一样东西，白蛇：缘起 动漫版播放地址获取自网路，如有侵权请联系...</p><p><br></p>', 'admin', '2019-07-01 00:00:00', '0', 'admin', NULL, 'admin', NULL, NULL, '缘起 动漫版是黄家康的一部作品，由未知参演，当前被点击了200次，平均评分5.0。大家有什么好的影视作品欢迎留言投稿！\n失忆少女小白被捕蛇人阿宣所救，为了解开自己的身份谜团，小白在阿宣的帮助下开始寻找线当你真正爱一样东西的时候你就会发现语言多么的脆弱和无力。文字与感觉永远有隔阂。。两人历经艰难险阻，年少男女在一路冒险中萌生爱慕之情。同时，小白的蛇妖身份也逐渐揭开，一场围绕人与妖的纠葛爱情故事随之展开。', 0, 0, 0, 0, 0, 0, 0, 'Y', 'Y', 0.00, 5.0);
...
-- ----------------------------
INSERT INTO `wm_movie_actor` VALUES (4, 95, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 95, 'director');
INSERT INTO `wm_movie_actor` VALUES (1, 95, 'director');
INSERT INTO `wm_movie_actor` VALUES (25, 94, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 94, 'director');
INSERT INTO `wm_movie_actor` VALUES (24, 94, 'director');
INSERT INTO `wm_movie_actor` VALUES (4, 110, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 110, 'director');
INSERT INTO `wm_movie_actor` VALUES (1, 111, 'director');
INSERT INTO `wm_movie_actor` VALUES (3, 111, 'director');
INSERT INTO `wm_movie_actor` VALUES (4, 111, 'director');
INSERT INTO `wm_movie_actor` VALUES (5, 111, 'director');
INSERT INTO `wm_movie_actor` VALUES (3, 112, 'director');
INSERT INTO `wm_movie_actor` VALUES (1, 113, 'director');
INSERT INTO `wm_movie_actor` VALUES (3, 113, 'director');
INSERT INTO `wm_movie_actor` VALUES (4, 113, 'director');
INSERT INTO `wm_movie_actor` VALUES (5, 113, 'director');
INSERT INTO `wm_movie_actor` VALUES (1, 115, 'director');
INSERT INTO `wm_movie_actor` VALUES (3, 115, 'director');
INSERT INTO `wm_movie_actor` VALUES (4, 115, 'director');
INSERT INTO `wm_movie_actor` VALUES (5, 115, 'director');
INSERT INTO `wm_movie_actor` VALUES (1, 116, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 116, 'actor');
INSERT INTO `wm_movie_actor` VALUES (4, 116, 'actor');
INSERT INTO `wm_movie_actor` VALUES (5, 116, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 117, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 117, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 120, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 120, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 121, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 121, 'actor');
INSERT INTO `wm_movie_actor` VALUES (4, 121, 'actor');
INSERT INTO `wm_movie_actor` VALUES (5, 121, 'actor');
INSERT INTO `wm_movie_actor` VALUES (1, 126, 'actor');
INSERT INTO `wm_movie_actor` VALUES (3, 126, 'actor');
INSERT INTO `wm_movie_actor` VALUES (4, 126, 'actor');
INSERT INTO `wm_movie_actor` VALUES (5, 126, 'actor');
...
) ENGINE = InnoDB AUTO_INCREMENT = 198 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '电影视频表' ROW_FORMAT = Dynamic;
